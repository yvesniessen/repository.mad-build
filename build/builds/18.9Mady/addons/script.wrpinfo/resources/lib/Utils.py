...
ADDON_PATH = xbmc.translatePath('special://home/addons/script.wrpinfo').decode('utf-8')
ADDON_DATA_PATH = xbmc.translatePath('special://profile/addon_data/script.wrpinfo').decode('utf-8')
IMAGES_DATA_PATH = xbmc.translatePath('special://profile/addon_data/script.wrpinfo/images').decode('utf-8')
SKIN_DIR = xbmc.getSkinDir()
AUTOPLAY_TRAILER = xbmcaddon.Addon().getSetting('autoplay_trailer')
NETFLIX_VIEW = xbmcaddon.Addon().getSetting('netflix_view')
OPENMETA_TV_FOLDER = xbmcaddon.Addon('plugin.video.wrp-metaplayer').getSetting('tv_library_folder') if xbmc.getCondVisibility('System.HasAddon(plugin.video.wrp-metaplayer)') else None
OPENMETA_MOVIE_FOLDER = xbmcaddon.Addon('plugin.video.wrp-metaplayer').getSetting('movies_library_folder') if xbmc.getCondVisibility('System.HasAddon(plugin.video.wrp-metaplayer)') else None
...
def log(txt):
	...
	message = u'script.wrpinfo:  %s' % txt
