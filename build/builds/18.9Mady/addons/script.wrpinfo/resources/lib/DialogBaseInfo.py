...
class DialogBaseInfo(object):
	...
	def fill_lists(self):
		for container_id, listitems in self.listitems:
			try:
				...
				Utils.log('Hinweis: Kein Container mit der ID %i verfügbar' % container_id)
	...
	def thumbnail_options(self):
		if not self.info.get('dbid'):
			return None
		selection = xbmcgui.Dialog().select(heading='Artwork', list=['Als Thumbnail verwenden'])
		if selection == 0:
	...
	def fanart_options(self):
		if not self.info.get('dbid'):
			return None
		selection = xbmcgui.Dialog().select(heading='Fanart', list=['Als Fanart verwenden'])
		if selection == 0:
			path = self.listitem.getProperty('original')
			media_type = self.window.getProperty('type')
	...
	def open_credit_dialog(self, credit_id):
		...
		if not listitems:
			listitems += [{'label': 'Keine Information verfügbar'}]
