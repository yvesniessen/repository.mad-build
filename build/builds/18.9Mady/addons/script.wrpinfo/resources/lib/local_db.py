...
def handle_db_movies(movie):
	trailer = 'plugin://script.wrpinfo?info=playtrailer&&dbid=%s' % movie['movieid']
	path = 'plugin://script.wrpinfo?info=extendedinfo&&dbid=%s' % movie['movieid']
...
def handle_db_tvshows(tvshow):
	path = 'plugin://script.wrpinfo?info=extendedtvinfo&&dbid=%s' % tvshow['tvshowid']
	db_tvshow = {
		'mediatype': 'tvshow',
