...
class DialogBaseList(object):
	...
	def reset_filters(self):
		if len(self.filters) > 0:
			...
			listitems.append('Entferne alle Filter')
			index = xbmcgui.Dialog().select(heading='Filter entfernen', list=listitems)
			if index == -1:
				return None
	...
	def update_ui(self):
		...
		if self.order == 'asc':
			self.setProperty('Order_Label', 'Aufsteigend')
		else:
			self.setProperty('Order_Label', 'Absteigend')
	...
	def add_filter(self, key, value, typelabel, label, force_overwrite=False):
		...
		ret = dialog.yesno(heading='Filter', line1='Wählen Sie das Filterverhalten', nolabel='Nein', yeslabel='Ja')
		if ret:
