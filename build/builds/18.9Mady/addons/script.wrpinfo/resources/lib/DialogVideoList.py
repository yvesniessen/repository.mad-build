...
SORTS = {
	...
	'tv': {
		...
		'vote_average': 'Stimmendurchnitt',
		'vote_count': 'Stimmenzahl',
		'first_air_date': 'Zuerst ausgestrahlt'
		}}
LANGUAGES = [
...
def get_tmdb_window(window_type):
	...
	class DialogVideoList(DialogBaseList, window_type):
		...
		def __init__(self, *args, **kwargs):
			...
			self.sort_label = kwargs.get('sort_label', 'Popularität')
		...
		def context_menu(self):
			...
			if self.listitem.getProperty('TVShowTitle'):
				listitems = ['Erste Episode abspielen']
			else:
				listitems = ['Abspielen']
			if self.listitem.getProperty('dbid'):
				listitems += ['Aus Bibliothek entfernen']
			else:
				listitems += ['Zur Bibliothek hinzufügen']
			listitems += ['Vorschau Trailer']
			selection = xbmcgui.Dialog().select(heading='Wähle eine der Optionen', list=listitems)
			if selection == 0:
				if self.listitem.getProperty('TVShowTitle'):
					url = 'plugin://plugin.video.wrp-metaplayer/tv/play/%s/1/1' % tvdb_id
					...
					if self.listitem.getProperty('dbid'):
						...
						dbid = 0
						url = 'plugin://plugin.video.wrp-metaplayer/movies/play/tmdb/%s' % item_id
					PLAYER.play_from_button(url, listitem=None, window=self, type='movieid', dbid=dbid)
			if selection == 1:
				if self.listitem.getProperty('TVShowTitle'):
					TVLibrary = xbmcaddon.Addon('plugin.video.wrp-metaplayer').getSetting('tv_library_folder')
					if self.listitem.getProperty('dbid'):
						...
						if os.path.exists(xbmc.translatePath('%s%s/' % (TVLibrary, tvdb_id))):
							...
							Utils.notify(header='[B]%s[/B]' % self.listitem.getProperty('TVShowTitle'), message='Aus Bibliothek entfernt', icon=self.listitem.getProperty('poster'), time=5000, sound=False)
							xbmc.sleep(250)
							self.update(force_update=True)
							self.getControl(500).selectItem(self.position)
					else:
						if xbmcgui.Dialog().yesno('WRPinfo', '[B]%s[/B] in der Bibliothek speichern?' % self.listitem.getProperty('TVShowTitle')):
							xbmc.executebuiltin('RunPlugin(plugin://plugin.video.wrp-metaplayer/tv/add_to_library/%s)' % tvdb_id)
							Utils.after_add(type='tv')
							Utils.notify(header='[B]%s[/B] wurde in der Bibliothek gespeichert' % self.listitem.getProperty('TVShowTitle'), message='Kodi beenden oder Bibliotheksaktualisierung starten', icon=self.listitem.getProperty('poster'), time=5000, sound=False)
				else:
					if self.listitem.getProperty('dbid'):
						if xbmcgui.Dialog().yesno('WRPinfo', '[B]%s[/B] aus der Bibliothek entfernen?' % self.listitem.getProperty('title')):
							Utils.get_kodi_json(method='VideoLibrary.RemoveMovie', params='{"movieid": %s}' % dbid)
							MovieLibrary = xbmcaddon.Addon('plugin.video.wrp-metaplayer').getSetting('movies_library_folder')
							if os.path.exists(xbmc.translatePath('%s%s/' % (MovieLibrary, imdb_id))):
								...
								Utils.notify(header='[B]%s[/B]' % self.listitem.getProperty('title'), message='aus Bibliothek entfernt', icon=self.listitem.getProperty('poster'), time=5000, sound=False)
								xbmc.sleep(250)
								self.update(force_update=True)
								self.getControl(500).selectItem(self.position)
					else:
						if xbmcgui.Dialog().yesno('WRPinfo', '[B]%s[/B] in der Bibliothek speichern?' % self.listitem.getProperty('title')):
							xbmc.executebuiltin('RunPlugin(plugin://plugin.video.wrp-metaplayer/movies/add_to_library/tmdb/%s)' % item_id)
							Utils.after_add(type='movie')
							Utils.notify(header='[B]%s[/B] wurde in der Bibliothek gespeichert' % self.listitem.getProperty('title'), message='Kodi beenden oder Bibliotheksaktualisierung starten', icon=self.listitem.getProperty('poster'), time=5000, sound=False)
			if selection == 2:
				if self.listitem.getProperty('TVShowTitle'):
					url = 'plugin://script.wrpinfo?info=playtvtrailer&&id=' + item_id
				else:
					url = 'plugin://script.wrpinfo?info=playtrailer&&id=' + item_id
		...
		def get_sort_type(self):
			...
			index = xbmcgui.Dialog().select(heading='Sortiere nach', list=listitems)
			if index == -1:
				return None
			if sort_strings[index] == 'vote_average':
				self.add_filter('vote_count.gte', '10', '%s (%s)' % ('Stimmenzahl', 'größer als'), '10')
		...
		def set_genre_filter(self):
			...
			index = xbmcgui.Dialog().select(heading='Wähle ein Genre', list=label_list)
			if index == -1:
				return None
		...
		def set_vote_count_filter(self):
			ret = True
			if not self.type == 'tv':
				ret = xbmcgui.Dialog().yesno(heading='Wähle eine der Optionen', line1='Wählen Sie das Filterverhalten', nolabel='Untere Grenze', yeslabel='Obere Grenze')
			result = xbmcgui.Dialog().input(heading='Stimmenzahl', type=xbmcgui.INPUT_NUMERIC)
			if result:
				if ret:
					self.add_filter('vote_count.lte', result, 'Stimmenzahl', ' < %s' % result)
				else:
					self.add_filter('vote_count.gte', result, 'Stimmenzahl', ' > %s' % result)
				self.mode = 'filter'
				self.page = 1
				self.update()
		...
		def set_year_filter(self):
			ret = xbmcgui.Dialog().yesno(heading='Wähle eine der Optionen', line1='Wählen Sie das Filterverhalten', nolabel='Untere Grenze', yeslabel='Obere Grenze')
			result = xbmcgui.Dialog().input(heading='Jahr', type=xbmcgui.INPUT_NUMERIC)
			if not result:
				return None
			if ret:
				order = 'lte'
				...
				label = ' > ' + result
			if self.type == 'tv':
				self.add_filter('first_air_date.%s' % order, value, 'Zuerst ausgestrahlt', label)
			else:
				self.add_filter('primary_release_date.%s' % order, value, 'Jahr', label)
			self.mode = 'filter'
			self.page = 1
			self.update()
		...
		def set_actor_filter(self):
			result = xbmcgui.Dialog().input(heading='Bitte Suchbegriff eingeben ...', type=xbmcgui.INPUT_ALPHANUM)
		...
		def set_company_filter(self):
			...
			if len(response) > 1:
				selection = xbmcgui.Dialog().select(heading='Wähle ein Studio', list=[item['name'] for item in response])
				if selection > -1:
					response = response[selection]
			elif response:
				response = response[0]
			else:
				Utils.notify('Kein Unternehmen gefunden')
		...
		def set_keyword_filter(self):
			...
			if not response:
				return None
			self.add_filter('with_keywords', str(response['id']), 'Schlüsselwort', response['name'])
			self.mode = 'filter'
			self.page = 1
			self.update()
		...
		def set_certification_filter(self):
			...
			index = xbmcgui.Dialog().select(heading='Landesvorwahl', list=country_list)
			if index == -1:
				return None
			country = country_list[index]
			cert_list = ['%s  -  %s' % (i['certification'], i['meaning']) for i in response[country]]
			index = xbmcgui.Dialog().select(heading='Wähle eine Zertifizierung', list=cert_list)
			if index == -1:
				return None
			cert = cert_list[index].split('  -  ')[0]
			self.add_filter('certification_country', country, 'Land der Zertifizierung', country)
			self.add_filter('certification', cert, 'Zertifizierung', cert)
			self.mode = 'filter'
			self.page = 1
			self.update()
		...
		def set_language_filter(self):
			...
			index = xbmcgui.Dialog().select(heading='Wähle eine Sprache', list=names)
			if index == -1:
				return None
			id = ids[index]
			name = names[index]
			if 'with_original_language' in [i['type'] for i in self.filters]:
				self.filters = []
			self.add_filter('with_original_language', id, 'Original Sprache', name)
			self.mode = 'filter'
			self.page = 1
			self.update()
		...
		def fetch_data(self, force=False):
			...
			if self.type == 'tv':
				temp = 'tv'
				rated = 'Bewertete Fernsehsendungen'
				starred = 'Markierte Fernsehsendungen'
			else:
				temp = 'movies'
				rated = 'Bewertete Filme'
				starred = 'Markierte Filme'
			if self.mode == 'search':
				...
				if self.search_str:
					self.filter_label = 'Suchergebnisse für:  ' + self.search_str
				else:
					self.filter_label = ''
			elif self.mode == 'list':
			...
			if not response['results']:
				Utils.notify('keine Ergebnisse gefunden')
			if self.mode == 'search':
