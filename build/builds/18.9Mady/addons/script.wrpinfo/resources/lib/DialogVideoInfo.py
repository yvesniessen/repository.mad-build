...
def get_movie_window(window_type):
	...
	class DialogVideoInfo(DialogBaseInfo, window_type):
		...
		def show_review(self):
			...
			wm.open_textviewer(header='Handlung', text=text, color='FFFFFFFF')
		...
		def search_in_meta_by_title(self):
			url = 'plugin://plugin.video.wrp-metaplayer/movies/tmdb/search_term/%s/1/' % self.info.get('title', '')
			self.close()
			xbmc.executebuiltin('ActivateWindow(videos,%s,return)' % url)

		@ch.click(132)
		def show_plot(self):
			wm.open_textviewer(header='Handlung', text=self.info['Plot'], color='FFFFFFFF')
		...
		def play_movie(self):
			if self.dbid and int(self.dbid) > 0:
				...
				url = 'plugin://plugin.video.wrp-metaplayer/movies/play/tmdb/%s' % self.info.get('id', '')
		...
		def play_movie_choose_player(self):
			if self.dbid and int(self.dbid) > 0:
				...
				url = 'plugin://plugin.video.wrp-metaplayer/movies/play_choose_player/tmdb/%s/False' % self.info.get('id', '')
				xbmc.executebuiltin('RunPlugin(%s)' % url)
		...
		def add_movie_to_library(self):
			...
			if xbmcgui.Dialog().yesno('WRPinfo', '[B]%s[/B] in der Bibliothek speichern?' % self.info['title']):
				xbmc.executebuiltin('RunPlugin(plugin://plugin.video.wrp-metaplayer/movies/add_to_library/tmdb/%s)' % self.info.get('id', ''))
				Utils.after_add(type='movie')
				Utils.notify(header='[B]%s[/B] wurde in der Bibliothek gespeichert' % self.info['title'], message='Kodi beenden oder Bibliotheksaktualisierung starten', icon=self.info['poster'], time=5000, sound=False)

		@ch.click(19)
		def remove_movie_from_library(self):
			if not xbmc.getCondVisibility('System.HasAddon(plugin.video.wrp-metaplayer)'):
				xbmc.executebuiltin('RunPlugin(plugin://plugin.video.wrp-metaplayer/setup/total)')
			if xbmcgui.Dialog().yesno('WRPinfo', 'Entferne [B]%s[/B] aus der Bibliothek?' % self.info['title']):
				if os.path.exists(xbmc.translatePath('%s%s/' % (Utils.WRPMETA_MOVIE_FOLDER, self.info['imdb_id']))):
					Utils.get_kodi_json(method='VideoLibrary.RemoveMovie', params='{"movieid": %d}' % int(self.info['dbid']))
					shutil.rmtree(xbmc.translatePath('%s%s/' % (Utils.WRPMETA_MOVIE_FOLDER, self.info['imdb_id'])))
					Utils.after_add(type='movie')
					Utils.notify(header='[B]%s[/B] wurde aus der Bibliothek entfernt' % self.info.get('title', ''), message='Kodi beenden oder Bibliotheksaktualisierung starten', icon=self.info['poster'], time=5000, sound=False)
