...
def get_tvshow_window(window_type):
	...
	class DialogTVShowInfo(DialogBaseInfo, window_type):
		...
		def browse_tvshow(self):
			url = 'plugin://plugin.video.wrp-metaplayer/tv/tvdb/%s/' % self.info['tvdb_id']
			self.close()
		...
		def credit_dialog(self):
			selection = xbmcgui.Dialog().select(heading='Wähle eine der Optionen', list=['Schauspielerinformationen anzeigen', 'Show Schauspieler TV-Show Auftritte'])
			if selection == 0:
		...
		def show_manage_dialog(self):
			manage_list = []
			manage_list.append(["WRPinfo Einstellungen", 'Addon.OpenSettings("script.wrpinfo")'])
			manage_list.append(["WRP-MetaPlayer Einstellungen", 'Addon.OpenSettings("plugin.video.wrp-metaplayer")'])
			manage_list.append(["YouTube Einstellungen", 'Addon.OpenSettings("plugin.video.youtube")'])
			selection = xbmcgui.Dialog().select(heading='Einstellungen', list=[i[0] for i in manage_list])
			if selection > -1:
		...
		@ch.click(6002)
		def open_list(self):
			index = xbmcgui.Dialog().select(heading='Liste auswählen', list=['Markierte Fernsehsendungen', 'Bewertete Fernsehsendungen'])
		...
		def play_tvshow(self):
			url = 'plugin://plugin.video.wrp-metaplayer/tv/play/%s/1/1' % self.info['tvdb_id']
			xbmc.executebuiltin('RunPlugin(%s)' % url)

		@ch.action('contextmenu', 9)
		def play_tvshow_choose_player(self):
			url = 'plugin://plugin.video.wrp-metaplayer/tv/play_choose_player/%s/1/1/False' % self.info['tvdb_id']
			xbmc.executebuiltin('RunPlugin(%s)' % url)

		@ch.click(20)
		def add_tvshow_to_library(self):
			if not xbmc.getCondVisibility('System.HasAddon(plugin.video.wrp-metaplayer)'):
				xbmc.executebuiltin('RunPlugin(plugin://plugin.video.wrp-metaplayer/setup/total)')
			if xbmcgui.Dialog().yesno('WRPinfo', ' [B]%s[/B] in der Bibliothek speichern?' % self.info['TVShowTitle']):
				xbmc.executebuiltin('RunPlugin(plugin://plugin.video.wrp-metaplayer/tv/add_to_library/%s)' % self.info['tvdb_id'])
				Utils.after_add(type='tv')
				Utils.notify(header='[B]%s[/B] wurde in der Bibliothek gespeichert' % self.info['TVShowTitle'], message='Kodi beenden oder Bibliotheksaktualisierung starten', icon=self.info['poster'], time=5000, sound=False)

		@ch.click(21)
		def remove_tvshow_from_library(self):
			if not xbmc.getCondVisibility('System.HasAddon(plugin.video.wrp-metaplayer)'):
				xbmc.executebuiltin('RunPlugin(plugin://plugin.video.wrp-metaplayer/setup/total)')
			if xbmcgui.Dialog().yesno('WRPinfo', 'Entferne [B]%s[/B] aus der Bibliothek?' % self.info['TVShowTitle']):
				if os.path.exists(xbmc.translatePath('%s%s/' % (Utils.WRPMETA_TV_FOLDER, self.info['tvdb_id']))):
					Utils.get_kodi_json(method='VideoLibrary.RemoveTVShow', params='{"tvshowid": %s}' % int(self.info['dbid']))
					shutil.rmtree(xbmc.translatePath('%s%s/' % (Utils.WRPMETA_TV_FOLDER, self.info['tvdb_id'])))
					Utils.after_add(type='tv')
					Utils.notify(header='[B]%s[/B] wurde aus der Bibliothek entfernt' % self.info['TVShowTitle'], message='Kodi beenden oder Bibliotheksaktualisierung starten', icon=self.info['poster'], time=5000, sound=False)

		@ch.click(132)
		def open_text(self):
			wm.open_textviewer(header='Überblick', text=self.info['Plot'], color='FFFFFFFF')
