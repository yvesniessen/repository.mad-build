...
if len(ext_key) == 32:
	...
	API_key = 'd43483099723dd3f3a77682a14f3f109'
...
def handle_tmdb_movies(results=[], local_first=True, sortkey='year'):
	...
	for movie in results:
		...
		trailer = 'plugin://script.wrpinfo?info=playtrailer&&id=%s' % tmdb_id
		path = 'plugin://script.wrpinfo?info=extendedinfo&&id=%s' % tmdb_id
		listitem = {
...
def handle_tmdb_tvshows(results, local_first=True, sortkey='year'):
	...
	for tv in results:
		...
		newtv = {
			...
			'Trailer': 'plugin://script.wrpinfo?info=tvtrailer&&id=%s' % tmdb_id,
			'year': Utils.get_year(Utils.fetch(tv, 'first_air_date')),
			'media_type': 'tv',
			'mediatype': 'tvshow',
			'character': Utils.fetch(tv, 'character'),
			'path': 'plugin://script.wrpinfo?info=extendedtvinfo&&id=%s' % tmdb_id,
...
def handle_tmdb_seasons(results):
	...
	for season in results:
		...
		title = 'Specials' if season_number == '0' else u'Staffel %s' % season_number
		listitem = {
			'media_type': 'season',
			'mediatype': 'season',
			'title': title,
...
def handle_tmdb_misc(results):
	...
	for item in results:
		...
		listitem = {
			...
			'path': 'plugin://script.wrpinfo?info=listmovies&---id=%s' % Utils.fetch(item, 'id'),
...
def handle_tmdb_people(results):
	...
	for person in results:
		...
		newperson = {
			...
			'path': 'plugin://script.wrpinfo?info=extendedactorinfo&&id=%s' % str(person['id']),
...
def extended_movie_info(movie_id=None, dbid=None, cache_time=14):
	if not movie_id:
		return None
	session_str = ''
	response = get_tmdb_data('movie/%s?append_to_response=alternative_titles,credits,images,keywords,releases,videos,translations,similar,reviews,rating&include_image_language=de,null,%s&language=%s&%s' % (movie_id, xbmcaddon.Addon().getSetting('LanguageID'), xbmcaddon.Addon().getSetting('LanguageID'), session_str), cache_time)
	if not response:
	...
	de_cert = Utils.dictfind(response['releases']['countries'], 'iso_3166_1', 'DE')
	if de_cert:
		mpaa = de_cert['certification']
	...
	movie = {
		...
		'path': 'plugin://script.wrpinfo?info=extendedinfo&&id=%s' % movie_id,
		'trailer': 'plugin://script.wrpinfo?info=playtrailer&&id=%s' % movie_id
		}
...
def extended_tvshow_info(tvshow_id=None, cache_time=7, dbid=None):
	if not tvshow_id:
		return None
	session_str = ''
	response = get_tmdb_data('tv/%s?append_to_response=alternative_titles,content_ratings,credits,external_ids,images,keywords,rating,similar,translations,videos&language=%s&include_image_language=de,null,%s&%s' % (tvshow_id, xbmcaddon.Addon().getSetting('LanguageID'), xbmcaddon.Addon().getSetting('LanguageID'), session_str), cache_time)
	if not response:
		return False
	videos = handle_tmdb_videos(response['videos']['results']) if 'videos' in response else []
	tmdb_id = Utils.fetch(response, 'id')
	...
	if len(response.get('episode_run_time', -1)) > 1:
		...
		duration = ''
	de_cert = Utils.dictfind(response['content_ratings']['results'], 'iso_3166_1', 'DE')
	if de_cert:
		mpaa = de_cert['rating']
	...
	tvshow = {
		...
		'Votes': Utils.fetch(response, 'vote_count'),
		'Status': translate_status(Utils.fetch(response, 'status')),
		'path': 'plugin://script.wrpinfo?info=extendedtvinfo&&id=%s' % tvshow_id,
		'trailer': 'plugin://script.wrpinfo?info=playtvtrailer&&id=%s' % tvshow_id,
...
def extended_season_info(tvshow_id, season_number):
	if not tvshow_id or not season_number:
		return None
	session_str = ''
	tvshow = get_tmdb_data('tv/%s?append_to_response=alternative_titles,content_ratings,credits,external_ids,images,keywords,rating,similar,translations,videos&language=%s&include_image_language=de,null,%s&%s' % (tvshow_id, xbmcaddon.Addon().getSetting('LanguageID'), xbmcaddon.Addon().getSetting('LanguageID'), session_str), 99999)
	response = get_tmdb_data('tv/%s/season/%s?append_to_response=videos,images,external_ids,credits&language=%s&include_image_language=de,null,%s&' % (tvshow_id, season_number, xbmcaddon.Addon().getSetting('LanguageID'), xbmcaddon.Addon().getSetting('LanguageID')), 7)
	...
	season = {
		...
		'year': year,
		'season': season_number,
		'path': 'plugin://script.wrpinfo?info=seasoninfo&tvshow=%s&season=%s' % (Utils.fetch(tvshow, 'name'), season_number),
...
def extended_episode_info(tvshow_id, season, episode, cache_time=7):
	...
	session_str = ''
	tvshow = get_tmdb_data('tv/%s?append_to_response=alternative_titles,content_ratings,credits,external_ids,images,keywords,rating,similar,translations,videos&language=%s&include_image_language=de,null,%s&%s' % (tvshow_id, xbmcaddon.Addon().getSetting('LanguageID'), xbmcaddon.Addon().getSetting('LanguageID'), session_str), 99999)
	response = get_tmdb_data('tv/%s/season/%s/episode/%s?append_to_response=credits,external_ids,images,rating,videos&language=%s&include_image_language=de,null,%s&%s&' % (tvshow_id, season, episode, xbmcaddon.Addon().getSetting('LanguageID'), xbmcaddon.Addon().getSetting('LanguageID'), session_str), cache_time)
	tmdb_id = Utils.fetch(tvshow, 'id')
	TVShowTitle = Utils.fetch(tvshow, 'name')
	external_ids = Utils.fetch(tvshow, 'external_ids')
	year = Utils.get_year(Utils.fetch(tvshow, 'first_air_date')),
	...
	answer = {
		...
		'tvdb_id': tvdb_id,
		'actors': actors,
		'path': 'plugin://script.wrpinfo?info=extendedepisodeinfo&tvshow_id=%s&season=%s&episode=%s' % (tvshow_id, season, episode),
		'crew': crew,
...
def translate_status(status_string):
	translations = {
		'released': 'Erste Ausstrahlung',
		'post production': 'Nach Produktion',
		'in production': 'In Produktion',
		'ended': 'Serie abgeschlossen',
		'returning series': 'Serien Fortsetzung',
		'planned': 'Geplante Ausstrahlung',
        'canceled': 'Serie abgesetzt'
		}
...
def get_keywords(movie_id):
	response = get_tmdb_data('movie/%s?append_to_response=alternative_titles,credits,images,keywords,releases,videos,translations,similar,reviews,rating&include_image_language=de,null,%s&language=%s&' % (movie_id, xbmcaddon.Addon().getSetting('LanguageID'), xbmcaddon.Addon().getSetting('LanguageID')), 30)
	keywords = []
...
def get_set_movies(set_id):
	response = get_tmdb_data('collection/%s?language=%s&append_to_response=images&include_image_language=de,null,%s&' % (set_id, xbmcaddon.Addon().getSetting('LanguageID'), xbmcaddon.Addon().getSetting('LanguageID')), 14)
	if response:
		artwork = get_image_urls(poster=response.get('poster_path'), fanart=response.get('backdrop_path'))
		info = {
			'label': response['name'],
...
def get_tvshow_ids(tvshow_id=None, cache_time=14):
	if not tvshow_id:
		return None
	session_str = ''
	response = get_tmdb_data('tv/%s?append_to_response=external_ids&language=%s&include_image_language=de,null,%s&%s' % (tvshow_id, xbmcaddon.Addon().getSetting('LanguageID'), xbmcaddon.Addon().getSetting('LanguageID'), session_str), cache_time)
	if not response:
		return False
	external_ids = Utils.fetch(response, 'external_ids')
	return external_ids
