...
def get_actor_window(window_type):
	...
	class DialogActorInfo(DialogBaseInfo, window_type):
		...
		def open_tvshow_dialog(self):
			selection = xbmcgui.Dialog().select(heading='Wählen Sie eine der Optionen', list=['Informationen zur TV Serie anzeigen', 'Schauspieler TV Serien Auftritte'])
			if selection == 0:
		...
		@ch.click(132)
		def show_plot(self):
			wm.open_textviewer(header='Überblick', text=self.info['biography'], color=self.info['ImageColor'])
