	...
	if len(match) > 0:
		...
		if version > BUILDVERSION:
			if DISABLEUPDATE == 'false':
				wiz.log("[Suche Updates] [Installierte Version: %s] [Aktuelle Version: %s] Öffne Update Fenster" % (BUILDVERSION, version), xbmc.LOGNOTICE)
				notify.updateWindow(BUILDNAME, BUILDVERSION, version, icon, fanart)
			else: wiz.log("[Suche Updates] [Installierte Version: %s] [Aktuelle Version: %s] Update Fenster deaktiviert" % (BUILDVERSION, version), xbmc.LOGNOTICE)
		else: wiz.log("[Suche Updates] [Installierte Version: %s] [Aktuelle Version: %s]" % (BUILDVERSION, version), xbmc.LOGNOTICE)
	else: wiz.log("[Suche Updates] FEHLER: Build-Version kann in der Build-Textdatei nicht gefunden werden", xbmc.LOGERROR)
...
wiz.log("Löschen älterer zwischengespeicherter Textdateien")
wiz.flushOldCache()

wiz.log("[Auto Repo Installation] gestartet", xbmc.LOGNOTICE)
if AUTOINSTALL == 'Yes' and not os.path.exists(os.path.join(ADDONS, REPOID)):
	...
	if workingxml == True:
		...
		if len(ver) > 0:
			...
			if workingrepo == True:
				DP.create(ADDONTITLE,'Lade Repository herunter...','', 'Bitte Warten Sie ...')
				...
				try:
					...
					wiz.LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, name[0]), "[COLOR %s]Add-on geupdated[/COLOR]" % COLOR2, icon=os.path.join(ADDONS, REPOID, 'icon.png'))
				except:
					pass
				...
				xbmc.sleep(500)
				wiz.forceUpdate(True)
				wiz.log("[Auto Repo Installation] Erfolgreich installiert", xbmc.LOGNOTICE)
			else:
				wiz.LogNotify("[COLOR %s]Repository Installationsfehler[/COLOR]" % COLOR1, "[COLOR %s]Ungültige Url für Zip Datei![/COLOR]" % COLOR2)
				wiz.log("[Auto Repo Installation] Konnte keine funktionierende URL für das Repository erstellen. %s" % workingrepo, xbmc.LOGERROR)
		else:
			wiz.log("Fehlerhafte URL der Repository Zip", xbmc.LOGERROR)
	else:
		wiz.LogNotify("[COLOR %s]Repository Installationsfehler[/COLOR]" % COLOR1, "[COLOR %s]Ungültige addon.xml Datei![/COLOR]" % COLOR2)
		wiz.log("[Auto Repo Installation] Die Datei addon.xml kann nicht gelesen werden.", xbmc.LOGERROR)
elif not AUTOINSTALL == 'Yes': wiz.log("[Auto Repo Installation] Nicht aktiviert", xbmc.LOGNOTICE)
elif os.path.exists(os.path.join(ADDONS, REPOID)): wiz.log("[Auto Repo Installation] Repository bereits installiert")

wiz.log("[Auto Update Wizard] Started", xbmc.LOGNOTICE)
if AUTOUPDATE == 'Yes':
	wiz.wizardUpdate('startup')
else: wiz.log("[Auto Update Wizard] Nicht aktiviert", xbmc.LOGNOTICE)
			...
			if not id == False:
				try:
					...
					if id == NOTEID:
						...
						else: wiz.log("[Notifications] id[%s] Entfernen" % int(id), xbmc.LOGNOTICE)
					elif id > NOTEID:
						...
						wiz.log("[Notifications] Abgeschlossen", xbmc.LOGNOTICE)
				except Exception, e:
