...
def index():
    ...
    if DEVELOPER == 'true': addDir('Wizard Entwickler Menü', 'developer', icon=ICONEMEN, themeit=THEME1)
...
def buildMenu():
    ...
    if bf == False:
        ...
        if HIDESPACERS == 'No': addFile(wiz.sep(), '', themeit=THEME3)
        addFile('Url für Textdatei nicht gefunden!', '', icon=ICONBUILDS, themeit=THEME3)
    ...
    else: addFile('Textdatei für Builds nicht korrekt formatiert.', '', icon=ICONBUILDS, themeit=THEME3)
    setView('files', 'viewType')
...
def viewBuild(name):
    ...
    if bf == False:
        WORKINGURL = wiz.workingURL(BUILDFILE)
        addFile('URL für Textatei ungültig', '', themeit=THEME3)
    ...
    for version, url, minor, gui, kodi, themefile, icon, fanart, preview, adult, info, description in match:
        ...
        if warning == True:
            addFile('[I]Build wurde für Kodi Version %s erstellt!(installiert: %s)[/I]' % (str(kodi), str(KODIV)), '', fanart=fanart, icon=icon, themeit=THEME3)
...
def viewThirdList(number):
    ...
    if not work == True:
        addFile('URL für Textdatei ungültig', '', icon=ICONBUILDS, themeit=THEME3)
        addFile('%s' % WORKINGURL, '', icon=ICONBUILDS, themeit=THEME3)
    else:
    ...
    if name == 'kodi':
        ...
        for url, name, size, date in match2:
            ...
            try:
                ...
                wiz.log("Error on: %s" % name)
        if x == 0: addFile("Fehler Kodi Scraper ist derzeit nicht verfügbar.")
        ...
        if not TEMPAPKFILE == False:
            ...
            if len(match) > 0:
                ...
                for aname, section, url, icon, fanart, adult, description in match:
                    ...
                    if x == 0:
                        addFile("Es wurden noch keine Addons zu diesem Menü hinzugefügt!", '', themeit=THEME2)
            else: wiz.log("[APK Menu] FEHLER: Ungültiges Format.", xbmc.LOGERROR)
        else:
            wiz.log("[APK Menu] FEHLER: URL für APK-Liste funktioniert nicht.", xbmc.LOGERROR)
            addFile('URL für Textdatei ungültig', '', themeit=THEME3)
            addFile('%s' % APKWORKING, '', themeit=THEME3)
        return
    else: wiz.log("[APK Menu] Keine APK-Liste hinzugefügt.")
    setView('files', 'viewType')
    ...
    if not ADDONFILE == 'http://':
        ...
        if not TEMPADDONFILE == False:
            ...
            if len(match) > 0:
                ...
                wiz.log("[Addon Menu] FEHLER: Ungültiges Format.")
        else:
            wiz.log("[Addon Menu] FEHLER: URL für die Addon Liste funktioniert nicht.")
            addFile('URL für Textatei ungültig', '', themeit=THEME3)
            addFile('%s' % ADDONWORKING, '', themeit=THEME3)
    else: wiz.log("[Addon Menu] Keine Addon Liste hinzugefügt.")
    setView('files', 'viewType')

def packInstaller(name, url):
    if not wiz.workingURL(url) == True: wiz.LogNotify("[COLOR %s]Addon Installer[/COLOR]" % COLOR1, '[COLOR %s]%s:[/COLOR] [COLOR %s]Ungültige Zip-URL![/COLOR]' % (COLOR1, name, COLOR2)); return
            ...
            if len(match) > 0:
                for name, url, repository, repositoryxml, repositoryurl, icon, fanart, adult, description in match:
                    if os.path.exists(os.path.join(ADDONS, plugin)):
                        ...
                        selected = DIALOG.select("[COLOR %s]Addon bereits installiert, was möchten Sie tun?[/COLOR]" % COLOR2, do)
                        if selected == 0:
                            ...
                            if DIALOG.yesno(ADDONTITLE, "[COLOR %s]Möchten Sie die Addon Daten entfernen für:" % COLOR2, "[COLOR %s]%s[/COLOR]?[/COLOR]" % (COLOR1, plugin), yeslabel="[B][COLOR springgreen]Entfernen[/COLOR][/B]", nolabel="[B][COLOR red]Abbrechen[/COLOR][/B]"):
                    ...
                    if not repository.lower() == 'none' and not os.path.exists(repo):
                        wiz.log("Repository nicht installiert, möchten Sie es installieren?")
                        if DIALOG.yesno(ADDONTITLE, "[COLOR %s]Möchten Sie das Repository für [COLOR %s]%s[/COLOR]:" % (COLOR2, COLOR1, plugin), "[COLOR %s]%s[/COLOR] installieren?[/COLOR]" % (COLOR1, repository), yeslabel="[B][COLOR springgreen]Installieren[/COLOR][/B]", nolabel="[B][COLOR red]Abbrechen[/COLOR][/B]"):
                        ...
                        else: wiz.log("[Addon Installer] Repository für %s nicht installiert: %s" % (plugin, repository))
...
def installAddon(name, url):
    ...
    DP.update(0, title,'', '[COLOR %s]Bitte warten Sie ...[/COLOR]' % COLOR2)
    percent, errors, error = extract.all(lib,ADDONS,DP, title=title)
    DP.update(0, title,'', '[COLOR %s]Installiere Abhängigkeiten[/COLOR]' % COLOR2)
    installed(name)
...
def youtubeMenu(name=None, url=None):
    if not YOUTUBEFILE == 'http://':
        ...
        if not TEMPYOUTUBEFILE == False:
            ...
            wiz.log("[YouTube Menu] FEHLER: URL für YouTube-Liste funktioniert nicht.")
            addFile('URL für Textdatei ungültig', '', themeit=THEME3)
            addFile('%s' % YOUTUBEWORKING, '', themeit=THEME3)
    else: wiz.log("[YouTube Menu] Keine YouTube Liste hinzugefügt")
    setView('files', 'viewType')
...
def maintMenu(view=None):
    ...
    feq        = ['Immer', 'Täglich', 'Alle 3 Tage', 'Wöchentlich']
    ...
    addDir ('[I]<< Zurück zum Hauptmenü[/I]', icon=ICONMAINT, themeit=THEME2)
...
def runSpeedTest():
    try:
        ...
        wiz.log("[Speed Test] Fehler beim Ausführen des Geschwindigkeitstests")
        pass
...
def advancedWindow(url=None):
    if not ADVANCEDFILE == 'http://':
        ...
        if not TEMPADVANCEDFILE == False:
            ...
            else: wiz.log("[Advanced Settings] FEHLER: Ungültiges Format.")
...
def writeAdvanced(name, url):
    ...
    if ADVANCEDWORKING == True:
        if os.path.exists(ADVANCED): choice = DIALOG.yesno(ADDONTITLE, "[COLOR %s]Wollen Sie Ihre aktuelle AdvancedSettings.xml überschreiben [COLOR %s]%s[/COLOR]?[/COLOR]" % (COLOR2, COLOR1, name), yeslabel="[B][COLOR springgreen]Überschreiben[/COLOR][/B]", nolabel="[B][COLOR red]Abbrechen[/COLOR][/B]")
        ...
        if choice == 1:
            ...
            f.write(file)
            f.close()
            DIALOG.ok(ADDONTITLE, '[COLOR %s]AdvancedSettings.xml wurde erfolgreich überschrieben.  Klicken Sie auf OK um Kodi zu beenden.[/COLOR]' % COLOR2)
            wiz.killxbmc(True)
...
def systemInfo():
    ...
    addFile('[B]Geräteinformationen:[/B]', '', icon=ICONMAINT, themeit=THEME2)
    addFile('[COLOR %s]Gerätename:[/COLOR] [COLOR %s]%s[/COLOR]' % (COLOR1, COLOR2, data[0]), '', icon=ICONMAINT, themeit=THEME3)
...
def saveMenu():
    ...
    addFile('- Klicken Sie unten, um die Einstellungen zu ändern -', '', themeit=THEME3)
...
def fixUpdate():
    if KODIV < 17:
        ...
        try:
            ...
            wiz.log("Entfernen nicht möglich %s, DB löschen" % dbfile)
            wiz.purgeDb(dbfile)
    else:
...
def removeAddonMenu():
	...
	if len(addonnames) == 0:
		...
		return
	if KODIV > 16:
		selected = DIALOG.multiselect("%s: Wählen Sie die Addons aus die Sie enfernen wollen." % ADDONTITLE, addonnames)
	else:
		selected = []; choice = 0
		tempaddonnames = ["-- Klicken Sie hier zum fortfahren --"] + addonnames
		while not choice == -1:
			choice = DIALOG.select("%s: Wählen Sie die Addons aus die Sie enfernen wollen." % ADDONTITLE, tempaddonnames)
			if choice == -1: break
			elif choice == 0: break
			else: 
				choice2 = (choice-1)
...
def removeAddonDataMenu():
	if os.path.exists(ADDOND):
		...
		addFile('[COLOR red][B][Entferne][/B][/COLOR] alle Addon Daten für deinstallierte Addons', 'removedata', 'uninstalled', themeit=THEME2)
		...
		for folder in sorted(fold, key = lambda x: x):
			...
			if foldername in EXCLUDES: folderdisplay = '[COLOR springgreen][B][Geschützt][/B][/COLOR] %s' % folderdisplay
...
def changeFeq():
	feq        = ['Immer', 'Täglich', 'Alle 3 Tage', 'Wöchentlich']
	change     = DIALOG.select("[COLOR %s]Wie oft soll die automatische Bereinigung beim Start ausgeführt werden?[/COLOR]" % COLOR2, feq)
	if not change == -1: 
...
def developer():
	...
	addFile('Teste Erst Start Menü',                'testfirst',             themeit=THEME1)
...
def buildWizard(name, type, theme=None, over=False):
    ...
    if type == 'gui':
        if name == BUILDNAME:
            if over == True: yes = 1
            else: yes = DIALOG.yesno(ADDONTITLE, '[COLOR %s]Möchten Sie den GUI Fix anwenden für:' % COLOR2, '[COLOR %s]%s[/COLOR]?[/COLOR]' % (COLOR1, name), nolabel='[B][COLOR red]Abbrechen[/COLOR][/B]',yeslabel='[B][COLOR springgreen]Installieren[/COLOR][/B]')
        else:
            yes = DIALOG.yesno("%s - [COLOR red]WARNUNG!![/COLOR]" % ADDONTITLE, "[COLOR %s][COLOR %s]%s[/COLOR] WRP Build ist derzeit nicht installiert." % (COLOR2, COLOR1, name), "Möchten Sie den GUI Fix trotzdem anwenden?.[/COLOR]", nolabel='[B][COLOR red]Abbrechen[/COLOR][/B]',yeslabel='[B][COLOR springgreen]Installieren[/COLOR][/B]')
        if yes:
            buildzip = wiz.checkBuild(name,'gui')
            zipname = name.replace('\\', '').replace('/', '').replace(':', '').replace('*', '').replace('?', '').replace('"', '').replace('<', '').replace('>', '').replace('|', '')
            if not wiz.workingURL(buildzip) == True: wiz.LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), '[COLOR %s]GUI Fix: Ungültige Zip-URL![/COLOR]' % COLOR2); return
            if not os.path.exists(PACKAGES): os.makedirs(PACKAGES)
            ...
            else: todo = DIALOG.yesno(ADDONTITLE, "[COLOR %s]Der GUI Fix wurde installiert. Möchten Sie das Profil neuladen oder Kodi beenden?[/COLOR]" % COLOR2, yeslabel="[B][COLOR red]Profil neuladen[/COLOR][/B]", nolabel="[B][COLOR springgreen]Kodi beenden[/COLOR][/B]")
            if todo == 1: wiz.reloadFix()
            else: DIALOG.ok(ADDONTITLE, "[COLOR %s]Um die Änderungen zu speichern, müssen Sie jetzt Kodi beenden. Drücken Sie OK, um Kodi zu beenden.[/COLOR]" % COLOR2); wiz.killxbmc('true')
        else:
        ...
        if warning == True:
            yes_pressed = DIALOG.yesno("%s - [COLOR red]WARNUNG!![/COLOR]" % ADDONTITLE, '[COLOR %s]Es besteht die Möglichkeit, dass der Skin nicht richtig angezeigt wird' % COLOR2, 'Wenn Sie ein Kodi %s Build auf eine Kodi Version %s installieren kann es zu Problemen kommen.' % (wiz.checkBuild(name, 'kodi'), KODIV), 'Möchten Sie dennoch [COLOR %s]%s v%s[/COLOR]?[/COLOR] installieren ?' % (COLOR1, name, wiz.checkBuild(name,'version')), nolabel='[B][COLOR red]Abbrechen[/COLOR][/B]',yeslabel='[B][COLOR springgreen]Installieren[/COLOR][/B]')
        else:
            if not over == False: yes_pressed = 1
            else: yes_pressed = DIALOG.yesno(ADDONTITLE, '[COLOR %s]WRP Build Installation. Möchten Sie den Build' % COLOR2, '[COLOR %s]%s v%s[/COLOR] \nherunterladen und installieren?[/COLOR]' % (COLOR1, name, wiz.checkBuild(name,'version')), nolabel='[B][COLOR red]Abbbrechen[/COLOR][/B]',yeslabel='[B][COLOR springgreen]Installieren[/COLOR][/B]')
        if yes_pressed:
            wiz.clearS('build')
            buildzip = wiz.checkBuild(name, 'url')
            zipname = name.replace('\\', '').replace('/', '').replace(':', '').replace('*', '').replace('?', '').replace('"', '').replace('<', '').replace('>', '').replace('|', '')
            if not wiz.workingURL(buildzip) == True: wiz.LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), '[COLOR %s]Build Installation: Ungültige Zip Url![/COLOR]' % COLOR2); return
            if not os.path.exists(PACKAGES): os.makedirs(PACKAGES)
        ...
        if theme == None:
            ...
            if not themefile == 'http://' and wiz.workingURL(themefile) == True:
                ...
                if len(themelist) > 0:
                    if DIALOG.yesno(ADDONTITLE, "[COLOR %s]Dieser Build [COLOR %s]%s[/COLOR] kommt mit [COLOR %s]%s[/COLOR] zusätzlichen Theme(s)" % (COLOR2, COLOR1, name, COLOR1, len(themelist)), "Eine Theme Installation ist zwingend erforderlich! \nMöchten Sie ein Theme jetzt installieren?[/COLOR]", yeslabel="[B][COLOR springgreen]Installiere Theme(s)[/COLOR][/B]", nolabel="[B][COLOR red]Installation abbrechen[/COLOR][/B]"):
                        wiz.log("Theme Liste: %s " % str(themelist))
                        ret = DIALOG.select(ADDONTITLE, themelist)
                        wiz.log("Theme Installation ausgewählt: %s" % ret)
        ...
        else: installtheme = DIALOG.yesno(ADDONTITLE, '[COLOR %s]Möchten Sie das Theme:' % COLOR2, '[COLOR %s]%s[/COLOR]' % (COLOR1, theme), 'für den [COLOR %s]%s v%s[/COLOR] Build installieren?[/COLOR]' % (COLOR1, name, wiz.checkBuild(name,'version')), yeslabel="[B][COLOR springgreen]Installiere Theme(s)[/COLOR][/B]", nolabel="[B][COLOR red]Installation abbrechen[/COLOR][/B]")
        if installtheme:
            themezip = wiz.checkTheme(name, theme, 'url')
            zipname = name.replace('\\', '').replace('/', '').replace(':', '').replace('*', '').replace('?', '').replace('"', '').replace('<', '').replace('>', '').replace('|', '')
            if not wiz.workingURL(themezip) == True: wiz.LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), '[COLOR %s]Theme Installation: Ungültige Zip Url![/COLOR]' % COLOR2); return False
            if not os.path.exists(PACKAGES): os.makedirs(PACKAGES)
...
def thirdPartyInstall(name, url):
    if not wiz.workingURL(url):
        LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), '[COLOR %s]Ungültige URL für Build[/COLOR]' % COLOR2); return
    type = DIALOG.yesno(ADDONTITLE, "[COLOR %s]Möchten Sie eine[COLOR %s]Neu Installation[/COLOR] or [COLOR %s]Erst/Update Installation[/COLOR] für:[/COLOR]" % (COLOR2, COLOR1, COLOR1), "[COLOR %s]%s[/COLOR]" % (COLOR1, name), yeslabel="[B][COLOR springgreen]Neu Installation[/COLOR][/B]", nolabel="[B][COLOR red]Erst/Update Installation[/COLOR][/B]")
    if type == 1:
    ...
    else: todo = DIALOG.yesno(ADDONTITLE, "[COLOR %s]Möchten Sie [COLOR %s]Kodi beenden[/COLOR] oder das [COLOR %s]Profil neuladen[/COLOR]?[/COLOR]" % (COLOR2, COLOR1, COLOR1), yeslabel="[B][COLOR springgreen]Profil neuladen[/COLOR][/B]", nolabel="[B][COLOR red]Kodi beenden[/COLOR][/B]")
    if todo == 1: wiz.reloadFix()
    else: wiz.killxbmc(True)
...
def apkInstaller(apk, url):
    ...
    if wiz.platform() == 'android':
        yes = DIALOG.yesno(ADDONTITLE, "[COLOR %s]Möchten Sie die Datei:" % COLOR2, "[COLOR %s]%s[/COLOR] herunterladen und installieren?" % (COLOR1, apk), yeslabel="[B][COLOR springgreen]Installieren[/COLOR][/B]", nolabel="[B][COLOR red]Abbrechen[/COLOR][/B]")
        if not yes: wiz.LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), '[COLOR %s]Fehler: Installation abgebrochen[/COLOR]' % COLOR2); return
        display = apk
        if not os.path.exists(PACKAGES): os.makedirs(PACKAGES)
        if not wiz.workingURL(url) == True: wiz.LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), '[COLOR %s]APK Installer: Ungültige Apk Url![/COLOR]' % COLOR2); return
...
def romInstaller(name, url):
    ...
    if myroms == '':
        if DIALOG.yesno(ADDONTITLE, "[COLOR %s]Es scheint, dass Sie kein Setup für den Speicherort für Rom-Packs haben" % COLOR2, "Möchten Sie einen festlegen?[/COLOR]", yeslabel="[COLOR springgreen][B]Speicherort[/B][/COLOR]", nolabel="[COLOR red][B]Herunterladen[/B][/COLOR]"):
            wiz.openS()
            myroms = wiz.getS('rompath')
            if myroms == '': return
    yes = DIALOG.yesno(ADDONTITLE, "[COLOR %s]Sind Sie sicher, dass Sie herunterladen und extrahieren möchten [COLOR %s]%s[/COLOR] to:" % (COLOR2, COLOR1, name), "[COLOR %s]%s[/COLOR]" % (COLOR1, myroms), yeslabel="[B][COLOR springgreen]Herunterladen[/COLOR][/B]", nolabel="[B][COLOR red]Abbrechen[/COLOR][/B]")
    if not yes: wiz.LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), '[COLOR %s]FEHLER: Installation abgebrochen[/COLOR]' % COLOR2); return
    display = name
    if not os.path.exists(PACKAGES): os.makedirs(PACKAGES)
    if not wiz.workingURL(url) == True: wiz.LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), '[COLOR %s]APK Installer: Ungültige Rom Url![/COLOR]' % COLOR2); return
    DP.create(ADDONTITLE,'[COLOR %s][B]Lade herunter:[/B][/COLOR] [COLOR %s]%s[/COLOR]' % (COLOR2, COLOR1, display),'', 'Bitte warten Sie ...')
    ...
    if state in ['true', 'false']:
        ...
        if not state in ['includevideo', 'includeall'] and wiz.getS('includeall') == 'true':
            try:
                ...
                wiz.LogNotify("[COLOR %s]Cache umschalten[/COLOR]" % COLOR1, "[COLOR %s]Ungültige id: %s[/COLOR]" % (COLOR2, state))
        else:
...
def removeAddon(addon, name, over=False):
	if not over == False:
		...
		yes = DIALOG.yesno(ADDONTITLE, '[COLOR %s]Möchten Sie folgendes Addon wirklich löschen:'% COLOR2, 'Name: [COLOR %s]%s[/COLOR]' % (COLOR1, name), 'ID: [COLOR %s]%s[/COLOR][/COLOR]' % (COLOR1, addon), yeslabel='[B][COLOR green]Entfernen[/COLOR][/B]', nolabel='[B][COLOR red]Abbrechen[/COLOR][/B]')
	if yes == 1:
...
def removeAddonData(addon, name=None, over=False):
	if addon == 'all':
		...
		if DIALOG.yesno(ADDONTITLE, '[COLOR %s]Möchten Sie [COLOR %s]ALLE[/COLOR]Addon-Daten, die in Ihrem Userdata-Ordner gespeichert sind, für deinstallierte Addons entfernen?[/COLOR]' % (COLOR2, COLOR1), yeslabel='[B][COLOR green]Entfernen[/COLOR][/B]', nolabel='[B][COLOR red]Abbrechen[/COLOR][/B]'):
			total = 0
			...
			wiz.LogNotify('[COLOR %s]Bereinigen durchgeführt[/COLOR]' % COLOR1, '[COLOR %s]%s Ordner entfernt[/COLOR]' % (COLOR2, total))
		else: wiz.LogNotify('[COLOR %s]Entferne Addon Daten[/COLOR]' % COLOR1, '[COLOR %s]Abgebrochen[/COLOR]' % COLOR2)
	elif addon == 'empty':
		if DIALOG.yesno(ADDONTITLE, '[COLOR %s]Möchten Sie [COLOR %s]ALLE[/COLOR] leeren Addon Daten Ordner aus ihrem Userdata Ordner entfernen?[/COLOR]' % (COLOR2, COLOR1), yeslabel='[B][COLOR green]Entfernen[/COLOR][/B]', nolabel='[B][COLOR red]Abbrechen[/COLOR][/B]'):
		...
		if addon in EXCLUDES:
			wiz.LogNotify("[COLOR %s]Geschütztes Plugin[/COLOR]" % COLOR1, "[COLOR %s]Addon Daten dürfen nicht entfernt werden![/COLOR]" % COLOR2)
		elif os.path.exists(addon_data):  
			if DIALOG.yesno(ADDONTITLE, '[COLOR %s]Möchten Sie auch die Addon-Daten entfernen für:[/COLOR]' % COLOR2, '[COLOR %s]%s[/COLOR]' % (COLOR1, addon), yeslabel='[B][COLOR green]Entfernen[/COLOR][/B]', nolabel='[B][COLOR red]Abbrechen[/COLOR][/B]'):
				...
				wiz.log('Addon Daten für %s entfernt' % addon)
	wiz.refresh()
    ...
    if wiz.workingURL(BUILDFILE) == True:
        if wiz.checkBuild(name, 'url'):
            ...
            if extend == True:
                ...
                msg += "[COLOR %s]Extrahierte Größe:[/COLOR] [COLOR %s]%s[/COLOR][CR][CR]" % (COLOR2, COLOR1, wiz.convertSize(int(float(extracted))))
                msg += "[COLOR %s]Gepackte Größe:[/COLOR] [COLOR %s]%s[/COLOR][CR][CR]" % (COLOR2, COLOR1, wiz.convertSize(int(float(zipsize))))
        ...
        else: wiz.log("Ungültiger Build Name!")
...
def buildVideo(name):
    if wiz.workingURL(BUILDFILE) == True:
        ...
        else: wiz.log("[%s]URL für Videovorschau konnte nicht gefunden werden" % name)
    else: wiz.log("Build Text Datei funktioniiert nicht: %s" % WORKINGURL)
...
def manageSaveData(do):
    if do == 'import':
        ...
        source = DIALOG.browse(1, '[COLOR %s]Wählen Sie den Speicherort der Datei SaveData.zip aus[/COLOR]' % COLOR2, 'files', '.zip', False, False, HOME)
        ...
        x = 0
        overwrite = DIALOG.yesno(ADDONTITLE, "[COLOR %s]Möchten Sie lieber alle Save Data-Dateien überschreiben oder Sie nach jeder zu importierenden Datei fragen?[/COLOR]" % (COLOR2), yeslabel="[B][COLOR springgreen]Überschreiben[/COLOR][/B]", nolabel="[B][COLOR red]Abbrechen[/COLOR][/B]")
            ...
            for item in files:
                ...
                if os.path.exists(old):
                    if overwrite == 1: os.remove(old)
                        ...
                        if not DIALOG.yesno(ADDONTITLE, "[COLOR %s]Möchten Sie die Datei [COLOR %s]%s[/COLOR] ersetzen?" % (COLOR2, COLOR1, item), yeslabel="[B][COLOR springgreen]Ersetzen[/COLOR][/B]", nolabel="[B][COLOR red]Abbrechen[/COLOR][/B]"): continue
                        else: os.remove(old)
        ...
        if os.path.exists(super):
            ...
            if os.path.exists(old):
                cont = DIALOG.yesno(ADDONTITLE, "[COLOR %s]Möchten Sie den Ordner [COLOR %s]Super Favourites[/COLOR] Addon Daten mit dem neuen ersetzen?" % (COLOR2, COLOR1), yeslabel="[B][COLOR springgreen]Ersetzen[/COLOR][/B]", nolabel="[B][COLOR red]Abbrechen[/COLOR][/B]")
            else: cont = 1
            if cont == 1:
                wiz.cleanHouse(old)
        ...
        source = DIALOG.browse(3, '[COLOR %s]Wählen Sie aus, wohin Sie die gespeicherte Daten-Zip exportieren möchten.[/COLOR]' % COLOR2, 'files', '', False, True, HOME)
        source = xbmc.translatePath(source)
        ...
        zipf.close()
        if source == mybuilds:
            DIALOG.ok(ADDONTITLE, "[COLOR %s]Wählen Sie aus, wohin Sie die gespeicherte Daten-Zip exportieren möchten.[/COLOR]" % (COLOR2), "[COLOR %s]%s[/COLOR]" % (COLOR1, tempzip))
        else:
            try:
...
def freshStart(install=None, over=False):
    ...
    elif install == 'restore': yes_pressed=DIALOG.yesno(ADDONTITLE, "[COLOR %s]Eine Neuinstalllation wird durchgeführt!" % COLOR2, "Alle Addons und Daten werden gelöscht!!!!", "Vor der Installation des lokalen Backups?[/COLOR]", nolabel='[B][COLOR red]Abbrechen[/COLOR][/B]', yeslabel='[B][COLOR springgreen]Fortsetzen[/COLOR][/B]')
    elif install: yes_pressed=DIALOG.yesno(ADDONTITLE, "[COLOR %s]Eine Neuinstalllation wird durchgeführt!" % COLOR2, "Alle Addons und Daten werden gelöscht!!!!", "Herunterladen und Installation des [COLOR %s]%s[/COLOR] Builds durchführen?" % (COLOR1, install), nolabel='[B][COLOR red]Abbrechen[/COLOR][/B]', yeslabel='[B][COLOR springgreen]Fortsetzen[/COLOR][/B]')
    else: yes_pressed=DIALOG.yesno(ADDONTITLE, "[COLOR %s]Möchten Sie Ihre Daten wiederherstellen?" % COLOR2, "Kodi-Konfiguration auf Standardeinstellungen?[/COLOR]", nolabel='[B][COLOR red]Abbrechen[/COLOR][/B]', yeslabel='[B][COLOR springgreen]Fortsetzen[/COLOR][/B]')
    if yes_pressed:
        if not wiz.currSkin() in ['skin.confluence', 'skin.estuary']:
        ...
        DP.update(0, "[COLOR %s]Dateien und Ordner löschen:" % COLOR2)
                ...
                if name == 'sources.xml' and fold[-1] == 'userdata' and KEEPSOURCES == 'true': wiz.log("Quellen behalten: %s" % os.path.join(root, name), xbmc.LOGNOTICE)
                    ...
                    try:
                        ...
                        if not name.startswith('Textures13'):
                            wiz.log('Entfernen fehlgeschlagen, DB löschen', xbmc.LOGNOTICE)
        ...
        if over == True:
            ...
            else: todo = DIALOG.yesno(ADDONTITLE, "[COLOR %s]Möchten Sie [COLOR %s]Kodi beenden[/COLOR] oder das [COLOR %s]Profil neuladen[/COLOR]?[/COLOR]" % (COLOR2, COLOR1, COLOR1), yeslabel="[B][COLOR red]Profil neuladen[/COLOR][/B]", nolabel="[B][COLOR springgreen]Kodi beenden[/COLOR][/B]")
...
####THANKS GUYS @ NaN #######
def clearCache():
    if DIALOG.yesno(ADDONTITLE, '[COLOR %s]Möchten Sie den Cache leeren?[/COLOR]' % COLOR2, nolabel='[B][COLOR red]Abbrechen[/COLOR][/B]', yeslabel='[B][COLOR springgreen]Entfernen[/COLOR][/B]'):
        wiz.clearCache()

def clearFunctionCache():
    if DIALOG.yesno(ADDONTITLE, '[COLOR %s]Möchten Sie den URL Resolver Cache löschen?[/COLOR]' % COLOR2, nolabel='[B][COLOR red]Abbrechen[/COLOR][/B]', yeslabel='[B][COLOR springgreen]Entfernen[/COLOR][/B]'):
        wiz.clearFunctionCache()
        
def clearArchive():
    if DIALOG.yesno(ADDONTITLE, '[COLOR %s]Möchten Sie den \'Archive_Cache\' Ordner löschen?[/COLOR]' % COLOR2, nolabel='[B][COLOR red]Abbrechen[/COLOR][/B]', yeslabel='[B][COLOR springgreen]Entfernen[/COLOR][/B]'):
        wiz.clearArchive()

def totalClean():
    if DIALOG.yesno(ADDONTITLE, '[COLOR %s]Möchten Sie Cache, heruntergeladene Pakete und Miniaturansichten löschen?[/COLOR]' % COLOR2, nolabel='[B][COLOR red]Abbrechen[/COLOR][/B]',yeslabel='[B][COLOR springgreen]Entfernen[/COLOR][/B]'):
        wiz.clearCache()
        wiz.clearFunctionCache()
...
def clearThumb(type=None):
    ...
    else: choice = DIALOG.yesno(ADDONTITLE, '[COLOR %s]Möchten Sie die Ordner %s und die zugehörigen Miniaturansichten löschen?' % (COLOR2, latest), "Sie werden beim nächsten Start neu gefüllt[/COLOR]", nolabel='[B][COLOR red]Abbrechen[/COLOR][/B]', yeslabel='[B][COLOR springgreen]Entferne Thumbs[/COLOR][/B]')
    if choice == 1:
        try: wiz.removeFile(os.join(DATABASE, latest))
        except: wiz.log('Löschen fehlgeschlagen, DB wird gelöscht.'); wiz.purgeDb(latest)
...
def purgeDb():
	...
	if KODIV >= 16: 
		choice = DIALOG.multiselect("[COLOR %s]Wähle DB Datei zum entfernen[/COLOR]" % COLOR2, display)
		...
		choice = DIALOG.select("[COLOR %s]Wähle DB Datei zum entfernen[/COLOR]" % COLOR2, display)
...
def testnotify():
	...
	else: wiz.LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), "[COLOR %s]Ungültige URL für Benachrichtigung[/COLOR]" % COLOR2)
...
def emumenu():
	...
	if len(match) > 0:
		if wiz.platform() == 'android':
			...
			DIALOG.ok(ADDONTITLE, "[COLOR yellow]Bitte laden Sie sich RetroArch für die Windows Version herunter.[/COLOR]")
		elif wiz.platform() == 'linux':
			DIALOG.ok(ADDONTITLE, "[COLOR yellow]Bitte laden Sie sich RetroArch für die Linux Version herunter[/COLOR]")
