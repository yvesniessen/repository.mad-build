...
def getCacheSize():
	...
	if INCLUDEVIDEO == 'true':
		...
		else: log("Entferne Cache: Video-Cache löschen, der nicht aktiviert ist", xbmc.LOGNOTICE)
	return totalsize
	...
	if os.path.exists(dep):
		...
		for depends in match:
			if not 'xbmc.python' in depends:
				...
				if not DP is None:
					DP.update("","Überprüfe Abhängigkeiten [COLOR yellow]%s[/COLOR] für [COLOR yellow]%s[/COLOR]" % (depends, name),"")
				if os.path.exists(dependspath):
					toggleAddon(name, 'true')
			xbmc.sleep(100)

def toggleAdult():
	do = DIALOG.yesno(ADDONTITLE, "[COLOR %s]Würden Sie gerne alle P18 Addons[COLOR %s]Aktivieren[/COLOR] oder [COLOR %s]Deaktivieren[/COLOR] ?[/COLOR]" % (COLOR2, COLOR1, COLOR1), yeslabel="[B][COLOR springgreen]Aktivieren[/COLOR][/B]", nolabel="[B][COLOR red]Deaktivieren[/COLOR][/B]")
	...
	if len(found) > 0:
		if DIALOG.yesno(ADDONTITLE, "[COLOR %s]Möchten Sie eine Liste der Addons anzeigen, für die %s verwendet wurde?[/COLOR]" % (COLOR2, goto.replace('ing', 'ed')), yeslabel="[B][COLOR springgreen]View List[/COLOR][/B]", nolabel="[B][COLOR red]Cancel[/COLOR][/B]"):
			editlist = '[CR]'.join(found)
			TextBox(ADDONTITLE, "[COLOR %s]Hier ist eine Liste der Addons, bei denen %s für P18 Inhalte gilt:[/COLOR][CR][CR][COLOR %s]%s[/COLOR]" % (COLOR1, goto.replace('ing', 'ed'), COLOR2, editlist))
...
def toggleAddon(id, value, over=None):
	...
	if 'error' in response and over is None:
		...
		DIALOG.ok(ADDONTITLE, "[COLOR %s]Fehler %s [COLOR %s]%s[/COLOR]" % (COLOR2, v, COLOR1 , id), "Überprüfen Sie, ob die Addon-Liste aktuell ist, und versuchen Sie es erneut.[/COLOR]")
		forceUpdate()
...
def createQR():
	url = getKeyboard('', "%s: Geben Sie die URL für den QRCode ein." % ADDONTITLE)
	if url == "": LogNotify("[COLOR %s]Erstelle QR Code[/COLOR]" % COLOR1, '[COLOR %s]Erstelle QR Code abgebrochen![/COLOR]' % COLOR2); return
	if not url.startswith('http://') and not url.startswith('https://'): LogNotify("[COLOR %s]Erstelle QR Code[/COLOR]" % COLOR1, '[COLOR %s]Keine gültige URL![/COLOR]' % COLOR2); return
	if url == 'http://' or url == 'https://': LogNotify("[COLOR %s]Erstelle QR Code[/COLOR]" % COLOR1, '[COLOR %s]Keine gültige URL![/COLOR]' % COLOR2); return
	working = workingURL(url)
	if not working:
		if not DIALOG.yesno(ADDONTITLE, "[COLOR %s]Es sieht so aus, als ob Ihre Eingabe nicht funktioniert. Möchten Sie den Code trotzdem erstellen?[/COLOR]" % COLOR2, "[COLOR %s]%s[/COLOR]" % (COLOR1, working), yeslabel="[B][COLOR red]Erstelle QR Code[/COLOR][/B]", nolabel="[B][COLOR springgreen]Abbrechen[/COLOR][/B]"):
			return
	name = getKeyboard('', "%s: Geben Sie den Namen für den QRCode ein." % ADDONTITLE)
...
def cleanupBackup():
	...
	selected = DIALOG.select("%s: Wählen Sie die zu entfernenden Elemente aus 'MyBuilds'." % ADDONTITLE, list)

	if selected == -1:
		LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), "[COLOR %s]Bereinigung abgebrochen![/COLOR]" % COLOR2)
	elif selected == 0:
		if DIALOG.yesno(ADDONTITLE, "[COLOR %s]Möchten Sie alle Elemente in Ihrem Ordner 'My_Builds' bereinigen?[/COLOR]" % COLOR2, "[COLOR %s]%s[/COLOR]" % (COLOR1, MYBUILDS), yeslabel="[B][COLOR springgreen]Bereinigen[/COLOR][/B]", nolabel="[B][COLOR red]Abbrechen[/COLOR][/B]"):
		...
		if DIALOG.yesno(ADDONTITLE, "[COLOR %s]Möchten Sie [COLOR %s]%s[/COLOR] aus dem 'My_Builds' Ordner entfernen?[/COLOR]" % (COLOR2, COLOR1, list[selected]), "[COLOR %s]%s[/COLOR]" % (COLOR1, path), yeslabel="[B][COLOR springgreen]Bereinigen[/COLOR][/B]", nolabel="[B][COLOR red]Abbrechen[/COLOR][/B]"):
			if os.path.isfile(path):
				try:
					os.remove(path)
					passed = True
				except:
					log("Entfernen nicht möglich: %s" % path)
			else:
				cleanHouse(path)
				try:
					shutil.rmtree(path)
...
def convertSpecial(url, over=False):
	...
	DP.create(ADDONTITLE, "[COLOR %s]Ändern der physischen Pfade in Spezial" % COLOR2, "", "Bitte warten Sie ...[/COLOR]")
	for root, dirs, files in os.walk(url):
		for file in files:
			...
			if file.endswith(".xml") or file.endswith(".hash") or file.endswith("properies"):
				...
				if DP.iscanceled():
					DP.close()
					LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), "[COLOR %s]Ändern der physischen Pfade abgebrochen[/COLOR]" % COLOR2)
					sys.exit()
	DP.close()
	log("[Ändern der physischen Pfade zu Special: abgeschlossen!", xbmc.LOGNOTICE)
	if not over: LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), "[COLOR %s]Ändern der physischen Pfade zu Special: abgeschlossen![/COLOR]" % COLOR2)
...
def clearCrash():
	...
	for file in glob.glob(os.path.join(LOG, '*crashlog*.*')):
		files.append(file)
	if len(files) > 0:
		if DIALOG.yesno(ADDONTITLE, '[COLOR %s]Möchten Sie die Absturzprotokolle löschen?' % COLOR2, '[COLOR %s]%s[/COLOR] Dateien gefunden[/COLOR]' % (COLOR1, len(files)), yeslabel="[B][COLOR springgreen]Entfernen[/COLOR][/B]", nolabel="[B][COLOR red]Erhalten[/COLOR][/B]"):
			for f in files:
				os.remove(f)
...
def hidePassword():
	if DIALOG.yesno(ADDONTITLE, "[COLOR %s]Möchten Sie alle [COLOR %s]Kennwörter ausblenden[/COLOR] bei der Eingabe in den verschiedenen Addons?[/COLOR]" % COLOR2, yeslabel="[B][COLOR springgreen]Kennwörter ausblenden[/COLOR][/B]", nolabel="[B][COLOR red]Abbrechen[/COLOR][/B]"):
		count = 0
					...
					if 'pass' in line:
						if not 'option="hidden"' in line:
							try:
								...
								count += 1
								log("[Kennwörter ausblenden] gefunden in %s bei %s" % (sett.replace(HOME, ''), line), xbmc.LOGDEBUG)
							except:
								pass
				f2 = open(sett, mode='w'); f2.write(f); f2.close()
		LogNotify("[COLOR %s]Kennwörter ausblenden[/COLOR]" % COLOR1, "[COLOR %s]%s Artikel geändert[/COLOR]" % (COLOR2, count))
		log("[Kennwörter ausblenden] %s Artikel geändert" % count, xbmc.LOGNOTICE)
	else: log("[Kennwörter ausblenden] Abgebrochen", xbmc.LOGNOTICE)

def unhidePassword():
	if DIALOG.yesno(ADDONTITLE, "[COLOR %s]Möchten Sie alle [COLOR %s]Kennwörter zeigen[/COLOR] bei der Eingabe in den verschiedenen Addons?[/COLOR]" % (COLOR2, COLOR1), yeslabel="[B][COLOR springgreen]Kennwörter zeigen[/COLOR][/B]", nolabel="[B][COLOR red]Abbrechen[/COLOR][/B]"):
		count = 0
		for folder in glob.glob(os.path.join(ADDONS, '*/')):
			sett = os.path.join(folder, 'resources', 'settings.xml')
			if os.path.exists(sett):
					...
					if 'pass' in line:
						if 'option="hidden"' in line:
							try:
								...
								f.replace(line, change)
								count += 1
								log("[Kennwörter zeigen] gefunden in %s bei %s" % (sett.replace(HOME, ''), line), xbmc.LOGDEBUG)
							except:
								pass
				f2 = open(sett, mode='w'); f2.write(f); f2.close()
		LogNotify("[COLOR %s]Kennwörter zeigen[/COLOR]" % COLOR1, "[COLOR %s]%s Artikel geändert[/COLOR]" % (COLOR2, count))
		log("[Kennwörter zeigen] %s Artikel geändert" % count, xbmc.LOGNOTICE)
	else: log("[Kennwörter zeigen] Abgebrochen", xbmc.LOGNOTICE)
...
def wizardUpdate(startup=None):
	if workingURL(WIZARDFILE):
		try:
			...
			return
		if ver > VERSION:
			yes = DIALOG.yesno(ADDONTITLE, '[COLOR %s]Es gibt eine neue Version von [COLOR %s]%s[/COLOR]!' % (COLOR2, COLOR1, ADDONTITLE), 'Möchten Sie das Update herunterladen [COLOR %s]v%s[/COLOR]?[/COLOR]' % (COLOR1, ver), nolabel='[B][COLOR red]Später erinnern[/COLOR][/B]', yeslabel="[B][COLOR springgreen]Update Wizard[/COLOR][/B]")
			if yes:
...
def asciiCheck(use=None, over=False):
	if use is None:
		source = DIALOG.browse(3, '[COLOR %s]Wählen Sie den Ordner aus, den Sie scannen möchten[/COLOR]' % COLOR2, 'files', '', False, False, HOME)
		if over:
			yes = 1
		else:
			yes = DIALOG.yesno(ADDONTITLE,'[COLOR %s]Möchten Sie [COLOR %s]entfernen[/COLOR] alle Dateinamen mit Sonderzeichen oder möchten Sie lieber einfach [COLOR %s]scannen und anzeigen[/COLOR] die Resultate im Log?[/COLOR]' % (COLOR2, COLOR1, COLOR1), yeslabel='[B][COLOR springgreen]Entfernen[/COLOR][/B]', nolabel='[B][COLOR red]Scannen[/COLOR][/B]')
	else:
		source = use
		yes = 1
	...
	for base, dirs, files in os.walk(source):
		...
		for file in files:
			...
			DP.update(prog2,"[COLOR %s]Überprüfe auf non ASCII Dateien" % COLOR2,'[COLOR %s]%s[/COLOR]' % (COLOR1, d), 'Bitte warten Sie ...[/COLOR]')
			try:
				file.encode('ascii')
...
def defaultSkin():
	log("[Standard Skin Überprüfung]", xbmc.LOGNOTICE)
	...
	log("Öffne GUI Einstellungen", xbmc.LOGNOTICE)
	...
	if len(match) > 0:
		...
		log("[Standard Skin Überprüfung] Skin Name: %s" % skinname, xbmc.LOGNOTICE)
		log("[Standard Skin Überprüfung] Skin ID: %s" % skinid, xbmc.LOGNOTICE)
	...
	if os.path.exists(tempgui):
		...
		os.remove(tempgui)
	log("[Standard Skin Überprüfung] Ende", xbmc.LOGNOTICE)
...
def backUpOptions(type, name=""):
	...
	try:
		...
		return
	if type == "addon pack":
		if DIALOG.yesno(ADDONTITLE, "[COLOR %s]Möchten Sie eine Datensicherung diverser Kodi Addons durchführen?[/COLOR]" % COLOR2, nolabel="[B][COLOR red]Abbrechen[/COLOR][/B]", yeslabel="[B][COLOR springgreen]Erstellen[/COLOR][/B]"):
			if name == "":
				name = getKeyboard("","Bitte geben Sie einen Namen für die %s.zip ein!" % type)
			...
			try:
				...
				try:
					...
					log("Unmöglich zu erstellen %s.zip" % name, xbmc.LOGERROR)
					if DIALOG.yesno(ADDONTITLE, "[COLOR %s]Wir können nicht in das aktuelle Sicherungsverzeichnis schreiben. Möchten Sie den Speicherort ändern?[/COLOR]" % COLOR2, yeslabel="[B][COLOR springgreen]Verzeichniss ändern[/COLOR][/B]", nolabel="[B][COLOR red]Abbrechen[/COLOR][/B]"):
						openS()
						return
					else:
						return
			...
			for folder in sorted(fold, key = lambda x: x):
				...
				if os.path.exists(xml):
					...
					if len(match) > 0:
						...
						addonfolds.append(foldername)
			if KODIV > 16:
				selected = DIALOG.multiselect("%s: Wählen Sie die Addons aus, die Sie dem Backup hinzufügen möchten." % ADDONTITLE, addonnames)
				if selected is None: selected = []
			else:
				selected = []; choice = 0
				tempaddonnames = ["-- Klicken Sie hier um fortzufahren --"] + addonnames
				while not choice == -1:
					choice = DIALOG.select("%s: Wählen Sie die Addons aus, die Sie dem Backup hinzufügen möchten." % ADDONTITLE, tempaddonnames)
					if choice == -1: break
					elif choice == 0: break
					else:
						choice2 = (choice-1)
		...
		if DIALOG.yesno(ADDONTITLE, "[COLOR %s]Möchten Sie eine Datensicherung des aktuellen Builds durchführen?[/COLOR]" % COLOR2, nolabel="[B][COLOR red]Abbrechen[/COLOR][/B]", yeslabel="[B][COLOR springgreen]Backup[/COLOR][/B]"):
			if name == "":
				name = getKeyboard("","Bitte geben Sie einen Namen für die %s.zip ein!" % type)
				if not name: return False
			...
			for_progress  = 0
			ITEM          = []
			if not DIALOG.yesno(ADDONTITLE, "[COLOR %s]Möchten Sie ihren Addon Daten Ordner dem Backup hinzufügen?" % COLOR2, 'Dieser Ordner enthält [COLOR %s]ALLE[/COLOR] Addon-Einstellungen, einschließlich Passwörtern, dieser kann jedoch auch wichtige Informationen enthalten, wie z. B. Skin Shortcuts. Wir empfehlen Ihnen, [COLOR %s]manuell[/COLOR] die nicht benötigten Ordner aus den Addon Daten zu entfernen.' % (COLOR1, COLOR1), 'Das Addon [COLOR %s]%s[/COLOR] und die Addon Daten werden ignoriert.[/COLOR]' % (COLOR1, ADDON_ID), yeslabel='[B][COLOR springgreen]Hinzufügen[/COLOR][/B]',nolabel='[B][COLOR red]Nein, Weiter[/COLOR][/B]'):
				...
				for file in files:
					try:
						...
						try:
							...
							log("[Backup] Type = '%s': Sicherung nicht möglich %s" % (type, file), xbmc.LOGNOTICE)
							log("%s / %s" % (Exception, e))
						if DP.iscanceled():
							DP.close()
		...
		if name == "":
			guiname = getKeyboard("","Bitte geben Sie einen Namen für die %s.zip ein!" % type)
		...
		if os.path.exists(GUISETTINGS):
			try:
				...
				try:
					...
					log("Unmöglich zu erstellen %s_guisettings.zip" % guiname, xbmc.LOGERROR)
					if DIALOG.yesno(ADDONTITLE, "[COLOR %s]Wir können nicht in das aktuelle Sicherungsverzeichnis schreiben. Möchten Sie den Speicherort ändern?[/COLOR]" % COLOR2, yeslabel="[B][COLOR springgreen]Verzeichnis ändern[/COLOR][/B]", nolabel="[B][COLOR red]Abbrechen[/COLOR][/B]"):
						openS()
						return
					else:
						return
			try:
				...
				for fold in match:
					fd = os.path.split(fold[:-1])[1]
					if not fd in ['skin.confluence', 'skin.re-touch', 'skin.estuary', 'skin.estouchy']:
						if DIALOG.yesno(ADDONTITLE, "[COLOR %s]Möchten Sie den aktuellen Addon Daten Ordner des Skins zur GUI Fix Datei hinzufügen?[/COLOR]" % COLOR2, "Aktuelles Theme: [COLOR %s]%s[/COLOR]" % (COLOR1, fd), yeslabel="[B][COLOR springgreen]Hinzufügen[/COLOR][/B]", nolabel="[B][COLOR red]Abbrechen[/COLOR][/B]"):
							for base, dirs, files in os.walk(os.path.join(ADDOND,fold)):
								files[:] = [f for f in files if f not in exclude_files]
								for file in files:
									fn = os.path.join(base, file)
		...
		if not DIALOG.yesno('[COLOR %s]%s[/COLOR][COLOR %s]: Theme Backup[/COLOR]' % (COLOR1, ADDONTITLE, COLOR2), "[COLOR %s]Möchten Sie eine Datensicherung des aktuellen Theme erstellen?[/COLOR]" % COLOR2, yeslabel="[B][COLOR springgreen]Erstellen[/COLOR][/B]", nolabel="[B][COLOR red]Abbrechen[/COLOR][/B]"): LogNotify("Theme Backup", "Abgebrochen!"); return False
		if name == "":
			themename = getKeyboard("","Bitte geben Sie einen Namen für die %s.zip ein!" % type)
		...
		try:
			...
			try:
				...
				log("Unmöglich zu erstellen %s.zip" % themename, xbmc.LOGERROR)
				if DIALOG.yesno(ADDONTITLE, "[COLOR %s]Wir können nicht in das aktuelle Sicherungsverzeichnis schreiben. Möchten Sie den Speicherort ändern?[/COLOR]" % COLOR2, yeslabel="[B][COLOR springgreen]Verzeichnis ändern[/COLOR][/B]", nolabel="[B][COLOR red]Abbrechen[/COLOR][/B]"):
					openS()
					return
				else:
		...
		try:
			if not SKIN == 'skin.confluence':
				...
				if len(match2) > 1:
					if DIALOG.yesno('[COLOR %s]%s[/COLOR][COLOR %s]: Theme Backup[/COLOR]' % (COLOR1, ADDONTITLE, COLOR2), "[COLOR %s]Möchten Sie die Texturdateien durchgehen für?[/COLOR]" % COLOR2, "[COLOR %s]%s[/COLOR]" % (COLOR1, SKIN), yeslabel="[B][COLOR springgreen]Hinzufügen[/COLOR][/B]", nolabel="[B][COLOR red]Nein, Weiter[/COLOR][/B]"):
						skinfold = os.path.join(ADDONS, SKIN, 'media')
						match2 = glob.glob(os.path.join(skinfold,'*.xbt'))
						for xbt in match2:
							if DIALOG.yesno('[COLOR %s]%s[/COLOR][COLOR %s]: Theme Backup[/COLOR]' % (COLOR1, ADDONTITLE, COLOR2), "[COLOR %s]Möchten Sie die Texturdatei [COLOR %s]%s[/COLOR] dem Backup hinzufügen?" % (COLOR1, COLOR2, xbt.replace(skinfold, "")[1:]), "von [COLOR %s]%s[/COLOR][/COLOR]" % (COLOR1, SKIN), yeslabel="[B][COLOR springgreen]Hinzufügen[/COLOR][/B]", nolabel="[B][COLOR red]Nein, Weiter[/COLOR][/B]"):
								fn  = xbt
								fn2 = fn.replace(HOME, "")
								zipf.write(fn, fn2, zipfile.ZIP_DEFLATED)
				else:
					for xbt in match2:
						if DIALOG.yesno('[COLOR %s]%s[/COLOR][COLOR %s]: Theme Backup[/COLOR]' % (COLOR1, ADDONTITLE, COLOR2), "[COLOR %s]Möchten Sie die Texturdatei [COLOR %s]%s[/COLOR] dem Backup hinzufügen?" % (COLOR2, COLOR1, xbt.replace(skinfold, "")[1:]), "Aktuelles Theme: [COLOR %s]%s[/COLOR][/COLOR]" % (COLOR1, SKIN), yeslabel="[B][COLOR springgreen]Hinzufügen[/COLOR][/B]", nolabel="[B][COLOR red]Nein, Weiter[/COLOR][/B]"):
							fn  = xbt
							fn2 = fn.replace(HOME, "")
							zipf.write(fn, fn2, zipfile.ZIP_DEFLATED)
				ad_skin = os.path.join(ADDOND, SKIN, 'settings.xml')
				if os.path.exists(ad_skin):
					if DIALOG.yesno('[COLOR %s]%s[/COLOR][COLOR %s]: Theme Backup[/COLOR]' % (COLOR1, ADDONTITLE, COLOR2), "[COLOR %s]Möchten Sie die aktuelle Theme [COLOR %s]settings.xml[/COLOR] Datei aus dem [COLOR %s]/addon_data/[/COLOR] Ordner dem Backup hinzufügen?" % (COLOR2, COLOR1, COLOR1), "Aktuelles Theme: [COLOR %s]%s[/COLOR]"  % (COLOR1, SKIN), yeslabel="[B][COLOR springgreen]Hinzufügen[/COLOR][/B]", nolabel="[B][COLOR red]Nein, Weiter[/COLOR][/B]"):
				...
				if 'script.skinshortcuts' in match:
					if DIALOG.yesno('[COLOR %s]%s[/COLOR][COLOR %s]: Theme Backup[/COLOR]' % (COLOR1, ADDONTITLE, COLOR2), "[COLOR %s]Möchten Sie die [COLOR %s]Sicherungsdatei[/COLOR] für das Addon [COLOR %s]Skin Shortcuts[/COLOR] dem Backup hinzufügen?" % (COLOR2, COLOR1, COLOR1), yeslabel="[B][COLOR springgreen]Hinzufügen[/COLOR][/B]", nolabel="[B][COLOR red]Nein, Weiter[/COLOR][/B]"):
						for base, dirs, files in os.walk(os.path.join(ADDOND,'script.skinshortcuts')):
							files[:] = [f for f in files if f not in exclude_files]
							for file in files:
								fn = os.path.join(base, file)
								zipf.write(fn, fn[len(HOME):], zipfile.ZIP_DEFLATED)
			if DIALOG.yesno('[COLOR %s]%s[/COLOR][COLOR %s]: Theme Backup[/COLOR]' % (COLOR1, ADDONTITLE, COLOR2), "[COLOR %s]Möchten Sie den [COLOR %s]Skin Ordner incl. Hintergrundbilder[/COLOR] dem Backup hinzufügen?[/COLOR]" % (COLOR2, COLOR1), yeslabel="[B][COLOR springgreen]Hinzufügen[/COLOR][/B]", nolabel="[B][COLOR red]Nein, Weiter[/COLOR][/B]"):
				...
				if DIALOG.yesno('[COLOR %s]%s[/COLOR][COLOR %s]: Theme Backup[/COLOR]' % (COLOR1, ADDONTITLE, COLOR2), "[COLOR %s]Möchten Sie die [COLOR %s]%s[/COLOR] Datenbank dem Backup hinzufügen?[/COLOR]" % (COLOR2, COLOR1, text), yeslabel="[B][COLOR springgreen]Hinzufügen[/COLOR][/B]", nolabel="[B][COLOR red]Nein, Weiter[/COLOR][/B]"):
					zipf.write(os.path.join(DATABASE, text), '/userdata/Database/%s' % text, zipfile.ZIP_DEFLATED)
			if DIALOG.yesno('[COLOR %s]%s[/COLOR][COLOR %s]: Theme Backup[/COLOR]' % (COLOR1, ADDONTITLE, COLOR2), "[COLOR %s]Möchten Sie Kodi Addons dem Backup hinzufügen?[/COLOR]" % (COLOR2), yeslabel="[B][COLOR springgreen]Hinzufügen[/COLOR][/B]", nolabel="[B][COLOR red]Nein, Weiter[/COLOR][/B]"):
				fold = glob.glob(os.path.join(ADDONS, '*/'))
				addonnames = []; addonfolds = []
				for folder in sorted(fold, key = lambda x: x):
					foldername = os.path.split(folder[:-1])[1]
					...
					if os.path.exists(xml):
						...
						if len(match) > 0:
							...
							addonfolds.append(foldername)
				if KODIV > 16:
					selected = DIALOG.multiselect("%s: Wählen Sie die Addons aus, die Sie zur Zip hinzufügen möchten." % ADDONTITLE, addonnames)
					if selected == None: selected = []
				else:
					selected = []; choice = 0
					tempaddonnames = ["-- Klicken Sie hier um fortzufahren --"] + addonnames
					while not choice == -1:
						choice = DIALOG.select("%s: Wählen Sie die Addons aus, die Sie zur Zip hinzufügen möchten." % ADDONTITLE, tempaddonnames)
						if choice == -1: break
						elif choice == 0: break
						else:
							choice2 = (choice-1)
							...
							for depends in match:
								...
								for base, dirs, files in os.walk(os.path.join(ADDONS,depends)):
									...
									for file in files:
										...
										zipf.write(fn, fn[len(HOME):], zipfile.ZIP_DEFLATED)
										added.append(depends)
			if DIALOG.yesno('[COLOR %s]%s[/COLOR][COLOR %s]: Theme Backup[/COLOR]' % (COLOR1, ADDONTITLE, COLOR2), "[COLOR %s]Möchten Sie die aktuellen Kodi Einstellungen [COLOR %s](guisettings.xml)[/COLOR] dem Backup hinzufügen?[/COLOR]" % (COLOR2, COLOR1), yeslabel="[B][COLOR springgreen]Hinzufügen[/COLOR][/B]", nolabel="[B][COLOR red]Nein, weiter[/COLOR][/B]"):
		...
		if DIALOG.yesno(ADDONTITLE, "[COLOR %s]Möchten Sie wirklich die aktuellen Addon Daten hinzufügen?[/COLOR]" % COLOR2, nolabel="[B][COLOR red]Backup abbrechen[/COLOR][/B]", yeslabel="[B][COLOR springgreen]Backup starten[/COLOR][/B]"):
			if name == "":
				name = getKeyboard("","Bitte geben Sie einen Namen für die %s.zip ein!" % type)
				if not name: return False
				name = urllib.quote_plus(name)
			...
			try:
				...
				try:
					...
					log("Unmöglich zu erstellen %s_addondata.zip" % name, xbmc.LOGERROR)
					if DIALOG.yesno(ADDONTITLE, "[COLOR %s]Wir können nicht in das aktuelle Sicherungsverzeichnis schreiben. Möchten Sie den Speicherort ändern?[/COLOR]" % COLOR2, yeslabel="[B][COLOR springgreen]Verzeichnis ändern[/COLOR][/B]", nolabel="[B][COLOR red]Abbrechen[/COLOR][/B]"):
						openS()
						return
					else:
...
def restoreLocal(type):
	...
	try:
		...
		return
	file = DIALOG.browse(1, '[COLOR %s]Wähle das Backup das du wiederherstellen willst[/COLOR]' % COLOR2, 'files', '.zip', False, False, mybuilds)
...
def restoreExternal(type):
	source = DIALOG.browse(1, '[COLOR %s]Wähle das Backup das du wiederherstellen willst[/COLOR]' % COLOR2, 'files', '.zip', False, False)
...
def whiteList(do):
	...
	if   do == 'edit':
		...
		while not choice in [-1, 0]:
			choice = DIALOG.select("%s: Wähle Addons die auf die WhiteList sollen." % ADDONTITLE, tempaddonnames)
			if choice == -1: break
			elif choice == 0: break
			else: 
				choice2 = (choice-1)
		...
		list = whiteList('read')
		if len(list) > 0:
			msg = "Hier ist eine Liste deiner WhitList Addons (inklusive Abhängigkeiten) die nicht entfernt werden, wenn du einen Neustart machst oder der UserData Ordner von einem Build überschrieben wird.[CR][CR]"
			for item in list:
				try: name, id, fold = item
				except Exception, e: log(str(e))
		...
		source = DIALOG.browse(1, '[COLOR %s]Wähle eine WhiteList Datei zum importieren[/COLOR]' % COLOR2, 'files', '.txt', False, False, HOME)
		log(str(source))
		...
		with open(WHITELIST, 'a') as f:
			for item in lines:
				try:
					...
					log("Fehler beim hinzufügen: '%s' / %s" % (item, str(e)), xbmc.LOGERROR)
					continue
				...
				if not id in idList:
					...
					f.write(writing)
			LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), "[COLOR %s]%s Addons hinzugefügt[/COLOR]" % (COLOR2, count))
	elif do == 'export':
		source = DIALOG.browse(3, '[COLOR %s]Wähle wohin die WhiteList exportiert werden soll[/COLOR]' % COLOR2, 'files', '.txt', False, False, HOME)
		...
		if not DIALOG.yesno(ADDONTITLE, "[COLOR %s]Willst du die WhiteList bereinigen?" % COLOR2, "Dieser Prozess kann nicht ruckgängig gemacht werden.[/COLOR]", yeslabel="[B][COLOR lime]Ja, bereinigen[/COLOR][/B]", nolabel="[B][COLOR red]Abbrechen[/COLOR][/B]"):
		...
		try: 
			...
			LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), "[COLOR %s]Fehler beim Löschen der Whitelist![/COLOR]" % (COLOR2))
		...
		try:
			for root, dirs, files in os.walk(PACKAGES):
				...
				if file_count > 0:
					...
					if over: yes=1
					else: yes=DIALOG.yesno("[COLOR %s]Entferne heruntergeladene Pakete" % COLOR2, "[COLOR %s]%s[/COLOR] Dateien gefunden / [COLOR %s]%s[/COLOR] Größe." % (COLOR1, str(file_count), COLOR1, size), "Möchten Sie diese löschen?[/COLOR]", nolabel='[B][COLOR red]Abbrechen[/COLOR][/B]',yeslabel='[B][COLOR springgreen]Entferne heruntergeladene Pakete[/COLOR][/B]')
					if yes:
	...
	if len(match) > 0:
		...
		if len(bad) > 0:
			...
			if len(remove) > 0:
				...
				removed = len(remove)
				DIALOG.ok(ADDONTITLE, "[COLOR %s]Die Überprüfung der Quellen auf fehlerhafte Pfade wurde abgeschlossen" % COLOR2, "Funktionierende: [COLOR %s]%s[/COLOR] | Gehalten: [COLOR %s]%s[/COLOR] | Entfernt: [COLOR %s]%s[/COLOR][/COLOR]" % (COLOR2, COLOR1, alive, COLOR1, kept, COLOR1, removed))
...
def reloadFix(default=None):
	DIALOG.ok(ADDONTITLE, "[COLOR %s]WARNUNG: Manchmal kann es vorkommen das Kodi abstürzt beim Profil neu laden.[/COLOR]" % COLOR2)
	if not os.path.exists(PACKAGES): os.makedirs(PACKAGES)
	if default is None:
		lookandFeelData('save')
	redoThumbs()
