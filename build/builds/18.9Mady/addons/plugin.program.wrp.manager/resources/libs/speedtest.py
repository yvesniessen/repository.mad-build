...
def speedtest(
    ...
    line1 = '[COLOR %s]Starte Speed Test..[/COLOR]' % COLOR2
    dp.create('%s: [COLOR %s]Speed Test[/COLOR]' % (ADDONTITLE,
              COLOR1), line1)
    dp.update(0)
    print_('Abrufen der speedtest.net Konfiguration...')
    line2 = \
        '[COLOR %s]Abrufen der speedtest.net Konfiguration...[/COLOR]' \
        % COLOR2
    dp.update(2, line1, line2)
    try:
        config = getConfig()
    except URLError:
        print_('Speedtest Konfiguration kann nicht abgerufen werden')
        sys.exit(1)

    print_('Abrufen der speedtest.net Server Liste ...')
    line3 = '[COLOR %s]Abrufen der speedtest.net Server Liste ...[/COLOR]' \
        % COLOR2
    dp.update(4, line1, line2, line3)

    servers = closestServers(config['client'])

    print_('Teste Server von %(isp)s (%(ip)s)...' % config['client'])
    line1 = '[COLOR ' + COLOR2 + ']Teste Server von:[/COLOR] [COLOR ' \
        + COLOR1 + ']%(isp)s (%(ip)s)[/COLOR]' % config['client']
    dp.update(6, line1)

    print_('Auswahl des besten Servers basierend auf der Latenz ...')
    line2 = \
        '[COLOR %s]Auswahl des besten Servers basierend auf der Latenz ...[/COLOR]' \
        % COLOR2
    dp.update(8, '', line2)
    ...
    line2 = ('[COLOR ' + COLOR2
             + ']Server Standort: %(name)s [%(d)0.2f km]: %(latency)s ms[/COLOR]'
              % best).encode('utf-8', 'ignore')
    dp.update(10, '', line2)
    ...
    print_('Teste Download Geschwindigkeit ...', end='')
    line3 = '[COLOR %s]Teste Download Geschwindigkeit ...[/COLOR]' % COLOR2
    dp.update(15, '', '', line3)
    ...
    print_('[COLOR red]Teste Download Geschwindigkeit[/COLOR]', end='')
    line2 = \
        '[COLOR %s]Teste Download Geschwindigkeit:[/COLOR] [COLOR %s]%0.2f M%s/s[/COLOR]' \
        % (COLOR2, COLOR1, dlspeed / 1000 / 1000 * units[1], units[0])
    line3 = '[COLOR %s]Teste Upload Geschwindigkeit ...[/COLOR]' % COLOR2
    ...
    while ulspeed < 1:
        ...
        dp.update(65, '', '', '[COLOR ' + COLOR2
                  + ']Teste Upload Geschwindigkeit... [Attempt [/COLOR]'
                  + str(i) + ']')
        ulspeed = uploadSpeed(best['url'], sizes, simple)
        print_()
    ...
    line1 = line2
    line2 = \
        '[COLOR %s]Teste Upload Geschwindigkeit:[/COLOR] [COLOR %s]%0.2f M%s/s[/COLOR]' \
        % (COLOR2, COLOR1, ulspeed / 1000 / 1000 * units[1], units[0])
    line3 = '[COLOR %s]Ergebnisse erzielen...[/COLOR]' % COLOR2
