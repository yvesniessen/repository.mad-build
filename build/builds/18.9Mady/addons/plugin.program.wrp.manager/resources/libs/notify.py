...
def autoConfig(msg='', TxtColor='0xFFFFFFFF', Font='font12', BorderWidth=10):
	class MyWindow(xbmcgui.WindowDialog):
		...
		def __init__(self,msg='',L=0,T=0,W=1280,H=720,TxtColor='0xFFFFFFFF',Font='font12',BorderWidth=10):
			...
			header = '[COLOR %s]Advanced Settings Konfiguration[/COLOR]' % (COLOR2)
			...
			header = '[COLOR %s]Video Cache Größe[/COLOR]' % (COLOR2)
			...
			msg3 = "[COLOR %s]Anzahl der zum Puffern von Streams im Speicher verwendeten Bytes. Bei Einstellung auf [COLOR %s]0[/COLOR] wird der Cache anstelle des RAM auf die Festplatte geschrieben.  Hinweis: Für die hier festgelegte Speichergröße benötigt Kodi das 3-fache des verfügbaren Arbeitsspeichers. Wenn Sie diesen Wert zu hoch einstellen, kann Kodi abstürzen, wenn nicht genügend RAM zur Verfügung steht (1/3 des freien Speichers: [COLOR %s]%s[/COLOR])[/COLOR]" % (COLOR2, COLOR1, COLOR1, freeMemory)
			...
			msg3 = "[COLOR %s][B]curlclienttimeout[/B] ist die Zeit in Sekunden, wie lange es dauert, bis die Verbindung zu libcurl unterbrochen wird und [B]curllowspeedtime[/B] dies ist die Zeit in Sekunden, die libcurl benötigt, um eine Verbindung mit niedriger Geschwindigkeit zu berücksichtigen. Für langsamere Verbindungen auf 20 einstellen.[/COLOR]" % COLOR2
			...
			msg3 = "[COLOR %s]Der Wert dieser Einstellung ist ein Multiplikator des Standardgrenzwerts. Wenn Kodi eine typische Blueraydatei mit 36 Mbit/s lädt, benötigt sie ein Wert von 2 das sind mindestens 72 Mbit/s Netzwerkbandbreite. Anders als bei der RAM-Einstellung können Sie diesen Wert jedoch sicher erhöhen, so hoch Sie möchten, und Kodi stürzt nicht ab.[/COLOR]" % COLOR2
			...
			self.addControl(self.Header4)
			msg4 = "[COLOR %s]Diese Einstellung zwingt Kodi, einen Cache für alle Videodateien zu verwenden, einschließlich des lokalen Netzwerks, des Internets und sogar der lokalen Festplatte. Der Standardwert ist 0 und es werden nur Videos zwischengespeichert, die Internet-Dateipfade / -quellen verwenden.[/COLOR]" % COLOR2
			...
			self.Button0 = xbmcgui.ControlRadioButton(rightside+(BorderWidth*3), B1, (W/2)-(BorderWidth*4), 30, '0: Puffert alle Internet-Dateisysteme', font='font12', focusTexture=radiobgfocus, noFocusTexture=radiobgnofocus, focusOnTexture=radiofocus, noFocusOnTexture=radiofocus, focusOffTexture=radionofocus, noFocusOffTexture=radionofocus)
			self.Button1 = xbmcgui.ControlRadioButton(rightside+(BorderWidth*3), B2, (W/2)-(BorderWidth*4), 30, '1: Puffert alle Dateisysteme', font='font12', focusTexture=radiobgfocus, noFocusTexture=radiobgnofocus, focusOnTexture=radiofocus, noFocusOnTexture=radiofocus, focusOffTexture=radionofocus, noFocusOffTexture=radionofocus)
			self.Button2 = xbmcgui.ControlRadioButton(rightside+(BorderWidth*3), B3, (W/2)-(BorderWidth*4), 30, '2: Puffert nur echte Internet-Dateisysteme', font='font12', focusTexture=radiobgfocus, noFocusTexture=radiobgnofocus, focusOnTexture=radiofocus, noFocusOnTexture=radiofocus, focusOffTexture=radionofocus, noFocusOffTexture=radionofocus)
			self.Button3 = xbmcgui.ControlRadioButton(rightside+(BorderWidth*3), B4, (W/2)-(BorderWidth*4), 30, '3: Puffert nichts', font='font12', focusTexture=radiobgfocus, noFocusTexture=radiobgnofocus, focusOnTexture=radiofocus, noFocusOnTexture=radiofocus, focusOffTexture=radionofocus, noFocusOffTexture=radionofocus)
			...
			self.buttonCancel=xbmcgui.ControlButton(rightside+BorderWidth*2,T+H-40-BorderWidth,(W/2)-(BorderWidth*2),35,"Abbrechen",textColor="0xFF000000",focusedColor="0xFF000000",alignment=2,focusTexture=buttonfocus,noFocusTexture=buttonnofocus)
		...
		def doWrite(self):
			...
			if os.path.exists(ADVANCED):
				choice = DIALOG.yesno(ADDONTITLE, "[COLOR %s]Derzeit ist eine [COLOR %s]AdvancedSettings.xml[/COLOR] aktiv. Möchten Sie diese entfernen und fortfahren?[/COLOR]" % (COLOR2, COLOR1), yeslabel="[B][COLOR springgreen]Entferne Datei[/COLOR][/B]", nolabel="[B][COLOR red]Abbrechen[/COLOR][/B]")
				if choice == 0: return
...
def QautoConfig(msg='', TxtColor='0xFFFFFFFF', Font='font10', BorderWidth=10):
	class MyWindow(xbmcgui.WindowDialog):
		...
		def __init__(self,msg='',L=0,T=0,W=1280,H=720,TxtColor='0xFFFFFFFF',Font='font10',BorderWidth=10):
			...
			leftside = L+BorderWidth
			rightside = L+(W/2)-(BorderWidth*2)
			header = '[COLOR %s]Schnellkonfiguration Advanced Settings[/COLOR]' % (COLOR2)
			self.Header=xbmcgui.ControlLabel(L, top, W, 30, header, font='font13', textColor=TxtColor, alignment=0x00000002)
			self.addControl(self.Header)
			top += 30+BorderWidth
			freeMemory = int(float(wiz.getInfo('System.Memory(free)')[:-2])*.33)
			...
			self.videomin = 0; self.videomax = freeMemory if freeMemory < 2000 else 2000
			self.recommendedVideo = recMemory if recMemory < 500 else 500; self.currentVideo = self.recommendedVideo
			current1 = '[COLOR %s]Video Cache Größe[/COLOR]=[COLOR %s]%s MB[/COLOR]' % (COLOR1, COLOR2, self.currentVideo)
			recommended1 = '[COLOR %s]Video Cache Größe:[/COLOR] [COLOR %s]%s MB[/COLOR]' % (COLOR1, COLOR2, self.recommendedVideo)
			self.curlmin = 0; self.curlmax = 20
			self.recommendedCurl = 10; self.currentCurl = self.recommendedCurl
			curlpos = wiz.percentage(self.currentCurl, self.curlmax)
			...
			msgbox='[COLOR %s]iese Einstellungen werden in die advancesettings.xml geschrieben [/COLOR]\r\n\r\n%s\r\n%s\r\n%s\r\n%s' %(COLOR1, recommended4, recommended1, recommended3, recommended2)
		...
		def doWrite(self):
			buffermode = 2
			if os.path.exists(ADVANCED):
				choice = DIALOG.yesno(ADDONTITLE, "[COLOR %s]Derzeit ist eine [COLOR %s]AdvancedSettings.xml[/COLOR] aktiv. Möchten Sie diese entfernen und fortfahren?[/COLOR]" % (COLOR2, COLOR1), yeslabel="[B][COLOR green]Entferne Datei[/COLOR][/B]", nolabel="[B][COLOR red]Abbrechen[/COLOR][/B]")
				if choice == 0: return
				try: os.remove(ADVANCED)
				except: f = open(ADVANCED, 'w'); f.close()
			if KODIV < 17:
				...
				with open(ADVANCED, 'w+') as f:
					...
					f.write('</advancedsettings>\n')
				f.close()
				wiz.LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), '[COLOR %s]AdvancedSettings.xml wurde erfolgreich geschrieben.[/COLOR]' % COLOR2)
			self.CloseWindow()
		def onControl(self, control):
			if   control==self.buttonWrite: self.doWrite()
			elif control==self.buttonCancel:  self.doExit()
...
def apkInstaller(apk):
	...
	xbmc.executebuiltin('Skin.SetString(apkinstaller, %s wurde erfolgreich herunter geladen.[CR]Klicke bitte Installieren im nächsten Fenster!)' % apk)
...
def firstRun():
	class MyWindow(xbmcgui.WindowXMLDialog):
		def __init__(self, *args, **kwargs):
			...
			self.msg   = "Aktuell ist kein Kodi White Rabbit Build installiert.\n\nWähle 'Kodi Builds' um ein White Rabbit Build zu installieren oder 'Ignorieren' um das Fenster zu schliessen.\n\nDanke das du den %s verwendest." % (ADDONTITLE)
		...
		def doBuildMenu(self):
			wiz.log("[Suche Updates] [User Selected: Open Build Menu] [Next Check: %s]" % str(NEXTCHECK), xbmc.LOGNOTICE)
...
def notification(msg='', test=False):
	class MyWindow(xbmcgui.WindowXMLDialog):
		...
		def doRemindMeLater(self):
			...
			wiz.log("[Notification] NotifyID %s Erinnere mich später" % wiz.getS('noteid'), xbmc.LOGNOTICE)
			self.close()

		def doDismiss(self):
			if not test == True:
				wiz.setS("notedismiss","true")
			wiz.log("[Notification] NotifyID %s Ignorieren" % wiz.getS('noteid'), xbmc.LOGNOTICE)
			self.close()
...
def updateWindow(name='Testing Window', current='1.0', new='1.1', icon=ICON, fanart=FANART):
	class MyWindow(xbmcgui.WindowXMLDialog):
		def __init__(self, *args, **kwargs):
			...
			self.fanart = kwargs['fanart']
			self.msgupdate  = "Es gibt ein Update des installierten Builds:\n[COLOR %s]%s[/COLOR]\n\nAktuelle Version: v[COLOR %s]%s[/COLOR]\nNeuste Version: v[COLOR %s]%s[/COLOR]\n\n[COLOR %s]Empfohlene Vorgehensweise:\nBei Versionsnummer 1.9.x Update Installation \nBei Versionsnummer 1.x.1 Neu Installation[/COLOR]" % (COLOR1, self.name, COLOR1, self.current, COLOR1, self.new, COLOR1)
		...
		def doFreshInstall(self):
			wiz.log("[Suche Updates] [Installierte Version: %s] [Aktuelle Version: %s] [Benutzerwahl: Neu Installation]" % (BUILDVERSION, LATESTVERSION), xbmc.LOGNOTICE)
			wiz.log("[Suche Updates] [Nächster Check: %s]" % str(NEXTCHECK), xbmc.LOGNOTICE)
			wiz.setS('lastbuildcheck', str(NEXTCHECK))
			self.close()
		...
		def doNormalInstall(self):
			wiz.log("[Suche Updates] [Installierte Version: %s] [Aktuelle Version: %s] [Benutzerwahl: Update Installation]" % (BUILDVERSION, LATESTVERSION), xbmc.LOGNOTICE)
			wiz.log("[Suche Updates] [Nächster Check: %s]" % str(NEXTCHECK), xbmc.LOGNOTICE)
			wiz.setS('lastbuildcheck', str(NEXTCHECK))
			self.close()
		...
		def doIgnore(self):
			wiz.log("[Suche Updates] [Installierte Version: %s] [Aktuelle Version: %s] [Benutzerwahl: Ignoriere 3 Tage]" % (BUILDVERSION, LATESTVERSION), xbmc.LOGNOTICE)
			wiz.log("[Suche Updates] [Nächster Check: %s]" % str(THREEDAYS), xbmc.LOGNOTICE)
