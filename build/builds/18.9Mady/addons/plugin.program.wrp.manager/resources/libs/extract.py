...
def allWithProgress(_in, _out, dp, ignore, title):
	...
	try:
		...
		errors += 1; error += '%s\n' % e
		wiz.log('Fehler Überprüfe Zip Datei: %s' % str(e), xbmc.LOGERROR)
	...
	for item in zin.infolist():
		try:
			...
			wiz.log("[ASCII Check] Unzulässiges Zeichen in Datei gefunden: {0}".format(item.filename))
			continue
		...
		line2  = '[COLOR %s][B]Dateien:[/B][/COLOR] [COLOR %s]%s/%s[/COLOR] ' % (COLOR2, COLOR1, count, int(nFiles))
		line2 += '[COLOR %s][B]Größe:[/B][/COLOR] [COLOR %s]%s/%s[/COLOR]' % (COLOR2, COLOR1, wiz.convertSize(size), zipsize)
		...
		if skip == True: wiz.log("Überspringen: %s" % item.filename, xbmc.LOGNOTICE)
		else:
			try:
				...
				wiz.log('Fehler beim Extrahieren: %s(%s)' % (item.filename, str(e)), xbmc.LOGERROR)
				pass
		dp.update(prog, line1, line2, line3)
		if dp.iscanceled(): break
	if dp.iscanceled():
		dp.close()
		wiz.LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), "[COLOR %sExtrahieren abgebrochen][/COLOR]" % COLOR2)
		sys.exit()
