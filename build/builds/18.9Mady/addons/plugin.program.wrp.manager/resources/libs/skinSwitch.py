...
def swapUS():
	...
	if 'false' in response:
		...
		query = '{"jsonrpc":"2.0", "method":"Settings.SetSettingValue","params":{"setting":%s,"value":%s}, "id":1}' % (new, value)
		response = xbmc.executeJSONRPC(query)
		wiz.LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), '[COLOR %s]Unbekannte Quellen:[/COLOR] [COLOR %s]Aktiviert[/COLOR]' % (COLOR1, COLOR2))
		wiz.log("Unbekannte Quellen in Einstellugen gesetzt: %s" % str(response), xbmc.LOGDEBUG)
