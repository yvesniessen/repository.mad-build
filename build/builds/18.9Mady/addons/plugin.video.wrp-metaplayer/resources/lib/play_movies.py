...
def play_movie(tmdb_id, usedefault):
	...
	if usedefault == 'True':
		...
		for player in players:
			if player.title == default:
			 	players = [player]
	movie = Movies(tmdb_id).info(language=plugin.get_setting('LangID', str), append_to_response='external_ids,alternative_titles,credits,images,keywords,releases,translations,rating')
	...
	if link:
		movie = Movies(tmdb_id).info(language=plugin.get_setting('LangID', str))
