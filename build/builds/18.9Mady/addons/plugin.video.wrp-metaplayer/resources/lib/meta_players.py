...
def get_players(media, filters={}):
	...
	players = []
	players_path = 'special://profile/addon_data/plugin.video.wrp-metaplayer/Players/'
	...
	for file in files:
		...
		try:
			...
			xbmcgui.Dialog().ok( _("Invalid player: Player"), ' %s is invalid' % file)
...
def get_needed_langs(players):
	...
	for player in players:
		for command_group in player.commands:  
			for command in command_group:
				command_lang = command.get('language', 'de')
