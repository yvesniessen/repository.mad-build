...
def play_episode(id, season, episode, usedefault):
	...
	for lang in meta_players.get_needed_langs(players):
		...
		if episode_parameters is not None:
			...
			if trakt_ids['tmdb'] != None and trakt_ids['tmdb'] != '':
				...
				plugin.ok( _("Information not found for:"), '%s - S%sE%s' % (show_info['name'], season, episode))
			else:
				plugin.ok( _("Information not found for:"), '%s - S%sE%s' % (show_info['name'], season, episode))
		if trakt_ids != None:
	...
	if link:
		plugin.setProperty('plugin.video.wrp-metaplayer.data', json.dumps(
			{
				'dbid': dbid,
				'tvdb': id,
				'season': season,
...
def tmdb_play_episode(id, season, episode):
	...
	try:
		...
		dbid = None
	show = TV(id).info(language=plugin.get_setting('LangID', str), append_to_response='external_ids,images')
	...
	preason = TV_Seasons(id, season).info(language=plugin.get_setting('LangID', str), append_to_response='external_ids,images')
	...
	prepisode = TV_Episodes(id, season, episode).info(language=plugin.get_setting('LangID', str), append_to_response='external_ids,images')
	...
	for lang in meta_players.get_needed_langs(players):
		...
		if episode_parameters is not None:
			...
			if trakt_ids['trakt'] != None and trakt_ids['trakt'] != '':
				...
				msg = 'Keine TMDb Informationen gefunden für %s - S%sE%s' % (show_info['name'], season, episode)
				plugin.ok( _("Episode information not found"), msg)
		if trakt_ids != None:
			params[lang].update(trakt_ids)
		params[lang]['info'] = show_info
		params[lang] = text.to_unicode(params[lang])
	...
	for lang in meta_players.get_needed_langs(players):
		...
		if episode_parameters is not None:
			...
			if trakt_ids['tmdb'] != None and trakt_ids['tmdb'] != '' and tried != 'tmdb': 
				...
				msg = 'Keine TVDb oder TMDb Informationen gefunden für %s - S%sE%s' % (show_info['name'], season, episode)
				plugin.ok( _("Episode information not found"), msg)
			else:
				msg = 'Keine TMDb Informationen gefunden für %s - S%sE%s' % (show_info['name'], season, episode)
				plugin.ok( _("Episode information not found"), msg)
