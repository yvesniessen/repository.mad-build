...
def tmdb_movies_blockbusters(page, raw=False):
	...
	result = Discover().movie(language=plugin.get_setting('LangID', str), append_to_response='external_ids,videos', **{'page': page, 'sort_by': 'revenue.desc'})
	if raw:
		return result
	else:
...
def tmdb_movies_now_playing(page, raw=False):
	...
	result = Movies().now_playing(language=plugin.get_setting('LangID', str), page=page, append_to_response='external_ids,videos')
	if raw:
		return result
	else:
		return list_tmdb_movies(result)
...
def tmdb_movies_popular(page, raw=False):
	from resources.lib.TheMovieDB import Movies
	result = Movies().top_rated(language=plugin.get_setting('LangID', str), page=page, append_to_response='external_ids,videos')
	if raw:
		return result
	else:
		return list_tmdb_movies(result)
...
def tmdb_movies_search_term(term, page):
	...
	result = Search().movie(query=term, language=plugin.get_setting('LangID', str), page=page, append_to_response='external_ids,videos')
...
def tmdb_movies_genre(id, page, raw=False):
	...
	result = Genres(id).movies(id=id, language=plugin.get_setting('LangID', str), page=page)
	if raw:
		return result
	else:
		return list_tmdb_movies(result)
...
def movies_add_all_to_library(items, noscan=False):
	...
	batch_add_file.close()
	xbmc.executebuiltin('RunPlugin(plugin://plugin.video.wrp-metaplayer/movies/batch_add_to_library)')
...
def movies_batch_add_to_library():
	...
	if xbmcvfs.exists(movie_batch_file):
		try:
			...
			plugin.notify( _("Movies"), _("Not found"), plugin.get_addon_icon(), 3000)
		library_folder = lib_movies.setup_library(plugin.get_setting('movies_library_folder', unicode))
		for id in ids:
			if ',' in id:
				csvs = id.split(',')
...
def list_tmdb_movies(result):
	...
	if 'page' in result:
		...
		if pages > page:
			...
			items.append(
				{
					'label': '%s/%s  [I]Nächste Seite[/I]  >>' % (page, pages + 1),
...
def movies_play(src, id, usedefault='True'):
	...
	if playaction == 1 and xbmc.getCondVisibility('system.hasaddon(script.wrpinfo)') and not plugin.getProperty('infodialogs.active'):
		...
		xbmc.executebuiltin('RunScript(script.wrpinfo,info=extendedinfo,%s=%s)' % (src, id))
	else:
		movies_play_choose_player(src, id, usedefault)

@plugin.route('/movies/play_by_name/<name>/<lang>')
def movies_play_by_name(name, lang='de', usedefault='True'):
	tools.show_busy()
	from resources.lib.TheMovieDB import Search
...
def movies_play_by_name_choose_player(name, lang='de', usedefault='False'):
...
def make_movie_item(movie_info):
	...
	if xbmc.getCondVisibility('system.hasaddon(script.wrpinfo)'):
		context_menu = [
			('Vorschau Trailer', 'RunScript(script.wrpinfo,info=playtrailer,id=%s)' % id),
			('In der Bibliothek speichern','RunPlugin(%s)' % plugin.url_for('movies_add_to_library', src=src, id=id))]
	else:
		context_menu = [
			('In der Bibliothek speichern','RunPlugin(%s)' % plugin.url_for('movies_add_to_library', src=src, id=id))]
	try:
