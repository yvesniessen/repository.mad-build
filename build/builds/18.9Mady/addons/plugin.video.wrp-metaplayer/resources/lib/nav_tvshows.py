...
def make_tvshow_item(info):
	...
	if xbmc.getCondVisibility('system.hasaddon(script.wrpinfo)'):
		context_menu = [
			('WRPinfo', 'RunScript(script.wrpinfo,info=extendedtvinfo,tvdb_id=%s)' % tvdb_id),
...
def list_seasons_tvdb(id, flatten):
	...
	for (season_num, season) in show.items():
		...
		if flatten == 'none':
			if xbmc.getCondVisibility('system.hasaddon(script.wrpinfo)'):
				context_menu = [
					('WRPinfo', 'RunScript(script.wrpinfo,info=seasoninfo,tvshow=%s,season=%s)' % (show_info['name'], season_num))]
			else:
				context_menu = []
			properties = {}
			try:
