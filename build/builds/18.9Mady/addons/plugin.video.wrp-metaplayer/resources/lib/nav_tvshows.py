...
def list_trakt_tvshows(results, pages, page):
	...
	if pages > 1:
		...
		items.append(
			{
				'label': '%s/%s  [I]Nächste Seite[/I]  >>' % (page + 1, pages + 1),
...
def tvdb_tv_search_term(term, page):
	search_results = TVDB.search(term, language=plugin.get_setting('LangID', str))
	items = []
...
def list_trakt_search_items(results, pages, page):
	...
	if pages > 1:
		...
		items.append(
			{
				'label': '%s/%s  [I]Nächste Seite[/I]  >>' % (nextpage, pages),
				'path': plugin.url_for(nav_base.caller_name(), **args),
				'thumbnail': plugin.get_media_icon('item_next'),
				'fanart': plugin.get_addon_fanart()
			})
...
def tmdb_tv_genre(id, page, raw=False):
	...
	result = Discover().tv(with_genres=id, page=page, language=plugin.get_setting('LangID', str))
	if raw:
		return result
	else:
...
def tmdb_tv_on_the_air(page, raw=False):
	...
	result = TV().on_the_air(page=page, language=plugin.get_setting('LangID', str))
	if raw:
		return result
	else:
		return list_tvshows(result)
...
def tmdb_tv_most_popular(page, raw=False):
	from resources.lib.TheMovieDB import TV
	result = TV().popular(page=page, language=plugin.get_setting('LangID', str))
	if raw:
		return result
	else:
		return list_tvshows(result)
...
def get_tvdb_id_from_name(name, lang):
	...
	if not search_results:
		tools.hide_busy()
		plugin.ok( _("TV show not found. No show information found for "), ' %s in TVDB' % text.to_utf8(name))
	items = []
	for show in search_results:
		if show['seriesname'] == name:
			...
			items.append(show)
	if len(items) > 1:
		selection = plugin.select( _("Choose TV Show"), ['%s (%s)' % (text.to_utf8(s['seriesname']), s['year']) for s in items])
	else:
		selection = 0
	tools.hide_busy()
	if selection != -1:
...
def get_tvdb_id_from_imdb_id(imdb_id):
	...
	if not tvdb_id:
		plugin.ok( _("TV show not found. No show information found for "), ' %s in TVDB' % imdb_id)
	return tvdb_id
...
def tv_play(id, season, episode, usedefault='True'):
	...
	if playaction == 1 and xbmc.getCondVisibility('system.hasaddon(script.wrpinfo)') and not plugin.getProperty('infodialogs.active'):
		from resources.lib.play_base import action_cancel
		action_cancel()
		xbmc.executebuiltin('RunScript(script.wrpinfo,info=extendedtvinfo,tvdb_id=%s)' % (id))
	elif playaction == 2 and xbmc.getCondVisibility('system.hasaddon(script.wrpinfo)') and not plugin.getProperty('infodialogs.active'):
		from resources.lib.play_base import action_cancel
		action_cancel()
		xbmc.executebuiltin('RunScript(script.wrpinfo,info=extendedepisodeinfo,tvdb_id=%s,season=%s,episode=%s)' % (id, season, episode))
	else:
		play_tvshows.play_episode(id, season, episode, usedefault)

@plugin.route('/tv/play_by_name/<name>/<season>/<episode>/<lang>', options={'lang': 'de'})
...
@plugin.route('/tv/play_by_name_choose_player/<name>/<season>/<episode>/<lang>/<usedefault>', options={'lang': 'de'})
...
def tv_add_to_library_parsed(id, player):
	if id.startswith('tt'):
		try:
			...
			plugin.ok( _("TV show not found. No show information found for "), ' %s in TVDB' % id)
	...
	if lib_tvshows.add_tvshow_to_library(library_folder, show, player):
		plugin.setProperty('plugin.video.wrp-metaplayer.clean_library', 'true')
...
def tv_add_all_to_library(items, noscan=False):
	...
	batch_add_file.close()
	xbmc.executebuiltin('RunPlugin(plugin://plugin.video.wrp-metaplayer/tv/batch_add_to_library)')
...
def tv_batch_add_to_library():
	...
	if xbmcvfs.exists(tv_batch_file):
		try:
			...
			plugin.notify( _("TV Shows"), _("Not found"), plugin.get_addon_icon(), 3000)
		library_folder = lib_tvshows.setup_library(plugin.get_setting('tv_library_folder', unicode))
		ids_index = 0
		for id in ids:
			if id == None or id == 'None':
				...
				try:
					...
					plugin.notify( _("Failed to add"), '%s' % id, plugin.get_addon_icon(), 3000)
			ids_index += 1
...
def list_tvshows(response):
	...
	if 'page' in response:
		...
		if page < response['total_pages']:
			...
			items.append(
				{
					'label': '%s/%s [I]Nächste Seite[/I]  >>' % (page + 1, response['total_pages']),
					'path': plugin.url_for(nav_base.caller_name(), **args),
					'thumbnail': plugin.get_media_icon('item_next'),
					'fanart': plugin.get_addon_fanart()
				})
...
def make_tvshow_item(info):
	...
	if xbmc.getCondVisibility('system.hasaddon(script.wrpinfo)'):
		context_menu = [
			('WRPinfo TV-Serien Informationen', 'RunScript(script.wrpinfo,info=extendedtvinfo,tvdb_id=%s)' % tvdb_id),
			('Vorschau Trailer', 'RunScript(script.wrpinfo,info=playtvtrailer,tvdb_id=%s)' % tvdb_id),
			('In der Bibliothek speichern', 'RunPlugin(%s)' % plugin.url_for('tv_add_to_library', id=tvdb_id))]
	else:
		context_menu = [
			('In der Bibliothek speichern', 'RunPlugin(%s)' % plugin.url_for('tv_add_to_library', id=tvdb_id))]
	properties = {}
...
def list_seasons_tvdb(id, flatten):
	...
	for (season_num, season) in show.items():
		...
		if flatten == 'none':
			if xbmc.getCondVisibility('system.hasaddon(script.wrpinfo)'):
				context_menu = [
					('WRPinfo TV-Serien Informationen', 'RunScript(script.wrpinfo,info=seasoninfo,tvshow=%s,season=%s)' % (show_info['name'], season_num))]
			else:
				context_menu = []
			properties = {}
			try:
				...
				pass
			seasonitem = {  'label': 'Staffel %s' % season_num,
