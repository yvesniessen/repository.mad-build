...
def play_by_label(label):
	...
	base = 'RunPlugin(plugin://plugin.video.wrp-metaplayer'
	info = xbmc.getInfoLabel
	if selection   == 0:
		xbmc.executebuiltin('%s/movies/play_by_name/%s/de)' % (base, label))
	elif selection == 1:
		xbmc.executebuiltin('%s/tv/play_by_name/%s/%s/%s/%s/de)' % (base, info('ListItem.TVShowTitle'), info('ListItem.Season'), info('ListItem.Episode'), label))
...
def get_video_link(players, params):
	...
	try:
		if len(players) > 1:
			index = plugin.select( _("Choose Player"), [player.title for player in players])
			if index == -1:
				return None
		...
		if result:
			...
			if len(links) == 1:
				...
				index = plugin.select( _("Choose Player"), [x['label'] for x in links])
				if index > -1:
					selection = links[index]
		else:
			plugin.ok( _("Error"), _("Video not found"))
	finally:
		lister.stop()
	return selection
